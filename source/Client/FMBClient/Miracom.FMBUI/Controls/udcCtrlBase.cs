...
//                m_CtrlStatus.StyleChanged += new Miracom.FMBUI.clsCtrlStatus.CtrlStyleChangedEventHandler(new CtrlStyleChangedEventHandler( this.OnCtrlStyleChanged));
            ...
            //UserControl1? Dispose瑜??ъ젙?섑븯??援ъ꽦 ?붿냼 紐⑸줉???뺣━?⑸땲??
            ...
            private bool m_bHot = false; // Control???꾩옱 Hot ?곹깭?몄? ?щ?
            private bool m_bSelected = false; // Control???꾩옱 Selected ?곹깭?몄? ?щ?
            private bool m_bFocused = false; // Control???꾩옱 Selected?대㈃??媛??理쒓렐???좏깮???곹깭?몄? ?щ?
            private bool m_bPressed = false; // Control???꾩옱 Pressed ?곹깭?몄? ?щ?
            private bool m_bModified = false; // Control???섏젙?섏뿀?붿? ?щ?
            private bool m_bRefreshed = false; // Control??Refresh?섏뿀?붿? ?щ?
                ...
                try
                {
                    switch (e.PropertyName)
                    {
                        ...
                        case Enums.eCtrlProperty.PROP_SUBAREAID:
                            ...
                            SetToolTip();
                            break;
                        case Enums.eCtrlProperty.PROP_MOTHERESOURCEID:

                            SetModified(true);
                            SetToolTip();
                            break;
                        case Enums.eCtrlProperty.PROP_RESSHAPE:
                            SetModified(true);
                            break;

                        case Enums.eCtrlProperty.PROP_WIPRATE:
                            SetModified(true);
                            SetToolTip();
                            break;
                    }
            ...
            private void mnuCtrlBase_Popup(object sender, System.EventArgs e)
            {
                ...
                try
                {
                    if (IsDesignMode == true)
                    {
                        ...
                        else if(CtrlStatus.ToolType == Enums.eToolType.SubResource)
                        {
                            mnuDelete.Visible = true;
                            mnuUpdate.Visible = true;
                            mnuProperties.Visible = true;
                        ...
                        else if(CtrlStatus.ToolType == Enums.eToolType.Tool)
                        {
                            mnuDelete.Visible = true;
                            mnuUpdate.Visible = true;
                            mnuProperties.Visible = true;
                        ...
                        else if (CtrlStatus.ToolType == Enums.eToolType.Port)
                        {
                            mnuDelete.Visible = true;
                            mnuUpdate.Visible = true;
                            mnuProperties.Visible = true;
                    ...
                    if (IsModified == true || CtrlStatus.IsSaveFlag == true)
                    {
                        if (CtrlStatus.ToolType == Enums.eToolType.Resource)
                        {
                            ...
                            sToolTip = "Name : " + this.Name + "\r\n" + "Text : " + CtrlStatus.Text
                                ...
                                + CtrlStatus.ResourceType + "\r\n" + "Area ID : " + CtrlStatus.AreaID + "\r\n"
                                + "Sub Area ID : " + CtrlStatus.SubAreaID + "\r\n" + "Last Event : " + CtrlStatus.LastEvent;
                            //Stocker
                            if(CtrlStatus.ResShape == '2')
                            {
                                double d_wip = CtrlStatus.WIPRate * 100;
                                sToolTip += "\r\n" + "WIP : " + d_wip.ToString("##0.00") + "%";
                            }
                        ...
                        {
                            sToolTip = "Name : " + this.Name + "\r\n" + "Text : " + CtrlStatus.Text + "\r\n" + "Tool Status : " + CtrlStatus.PrimaryStatus + "\r\n" + "Resource Type : " + CtrlStatus.ResourceType + "\r\n" + "Last Event : " + CtrlStatus.LastEvent;
                            if (CtrlStatus.ToolTipComment != "")
                            {
                                sToolTip += "\r\n" + CtrlStatus.ToolTipComment;
                            }
                        ...
                        {
                            sToolTip = "Name : " + this.Name + "\r\n" + "Text : " + CtrlStatus.Text + "\r\n" +  "Res_ID : " + CtrlStatus.MotherResourceID + "\r\n"+ "Port Status : " + CtrlStatus.PrimaryStatus + "\r\n" + "Resource Type : " + CtrlStatus.ResourceType;
                            if (CtrlStatus.ToolTipComment != "")
                            {
                                sToolTip += "\r\n" + CtrlStatus.ToolTipComment;
                            }
            ...
            public void SetCtrlStatusData(clsCtrlStatus data, int iStep, bool bChange)
            {
                ...
                try
                {
                    if (iStep == 1) // All Property Update
                    {
                        ...
                        CtrlStatus.MotherResourceID = data.MotherResourceID;
                        CtrlStatus.ResShape = data.ResShape;
                        CtrlStatus.WIPRate = data.WIPRate;
                        CtrlStatus.SignalPosition = data.SignalPosition;
                    }
            ...
            public String GetMotherResourceID()
            {

                try
                {
                    return CtrlStatus.MotherResourceID;

                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message, "udcCtrlBase.GetMotherResourceID()", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return String.Empty;
                }
            }
            ...
            public object Clone(ref udcCtrlBase cloneData)
            {
                ...
                try
                {
                    ...
                    cloneData.CtrlStatus.ResShape = this.CtrlStatus.ResShape;
                    cloneData.CtrlStatus.WIPRate = this.CtrlStatus.WIPRate;
                    cloneData.CtrlStatus.SignalPosition = this.CtrlStatus.SignalPosition;
