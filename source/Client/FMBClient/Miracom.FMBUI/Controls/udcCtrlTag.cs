                    ...
                    switch (CtrlStatus.ToolType)
                    {
                        case Enums.eToolType.Rectangle:
                            ...
                            if (IsSelected == true && IsDesignMode == true)
                            {
                                ...
                                //gp.AddRectangle(CtrlPos);

                                //GraphicsPath path = (GraphicsPath)gp.Clone();

                                ////  create a new transformation matrix
                                //Matrix mTransform = new Matrix();

                                ////  rotate the matrix around the midpoint of the clock at the angle passed in.
                                //mTransform.RotateAt((float)45, new PointF((float)(CtrlPos.X + CtrlPos.Width / 2.0), (float)(CtrlPos.Y + CtrlPos.Height / 2.0)));

                                ////  Transform the Graphics Path (which is actually the clock hand)
                                //path.Transform(mTransform);
