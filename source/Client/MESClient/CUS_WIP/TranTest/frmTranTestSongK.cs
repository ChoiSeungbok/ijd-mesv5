...
namespace CUS_WIP
{
    public partial class frmTranTestSongK : Miracom.MESCore.TranForm02
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //공정정보 가져오기
            TRSNode in_node = new TRSNode("WO_IF_IN");
            TRSNode out_node = new TRSNode("WO_IF_OUT");

            MPCR.SetInMsg(in_node);


            if (MPCR.CallService("CUS_BAT", "CUS_BAT_Erp_Mes_Wo_Info", in_node, ref out_node) == false)
            {
                return;
            }
