...
namespace CUS_WIP
{
    public partial class frmTranTransferResInfo : CUS_COM.frmTranForm01
    {
        ...
        private bool SendResInfo(char c_step)
        {
            try
            {
                ...
                else if (c_step == '2')
                {
                    ...
                    in_node.AddString("IMP_LOT_ID", txtStandardLot1.Text);
                    in_node.AddString("IMP_RECIPE_ID", cdvRecipe1.Text);
                    ...
                    in_node.AddString("ONLY_CUP_YN", cdvOnlyCup.Text);
                }
        ...
        private void cdvOnlyCup_ButtonPress(object sender, EventArgs e)
        {
            ...
            {
                cdvOnlyCup.Init();
                MPCF.InitListView(cdvOnlyCup.GetListView);
                cdvOnlyCup.Columns.Add("OnlyCup", 50, HorizontalAlignment.Left);
                cdvOnlyCup.SelectedSubItemIndex = 0;

                if (BASLIST.ViewGCMDataList(cdvOnlyCup.GetListView, '1', MPGC.MP_GCM_YESNO) == false)
                {
                    return;
                }

                cdvOnlyCup.InsertEmptyRow(0, 1);
            }
            catch (Exception ex)
            {
                MPCF.ShowMsgBox(ex.Message);
