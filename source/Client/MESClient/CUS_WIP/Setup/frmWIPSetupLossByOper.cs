    ...
    public partial class frmWIPSetupLossByOper : Miracom.MESCore.SetupForm02
    {
        ...
        private void btnGCM_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                popup.sTable_name = "LOSS_CODE";

                if (MPGV.gcLanguage == '2')
                {
                    popup.sTable_name_desc = "손실코드 추가";
                    popup.sComment = "손실코드를 등록 합니다.(불량코드를 삭제하는경우 기존 불량이력도 확인하는데 문제가 발생하기떄문에 주의바랍니다.)";

                }
                else
                {
                    popup.sTable_name_desc = "Loss Code Add";
                    popup.sComment = "If you delete a defect code, please be careful because there may be a problem in checking the existing defect history.";
