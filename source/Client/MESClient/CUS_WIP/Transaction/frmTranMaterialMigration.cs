...
namespace CUS_WIP
{
    public partial class frmTranMaterialMigration : CUS_COM.frmViewForm01
    {
        ...
        private bool Create_Lot()
        {
            ...
            int i = 0, j = 0;
            int iMatCnt = 0, iWipCnt = 0, iSemiCnt = 0, iProdCnt = 0, iOutGoingCnt = 0, iMoldCnt = 0;
            int iCol = 0;

            try
            {
                if(spdLotList_Material.RowCount == 0 && spdLotList_WIP.RowCount == 0 &&  spdLotList_Semi_Goods.RowCount == 0 
                    && spdLotList_Finished_Goods.RowCount == 0 && spdLotList_Outgoing.RowCount == 0 && spdLotList_Mold.RowCount == 0)
                {
                ...
                if(iMatCnt + iWipCnt + iSemiCnt + iProdCnt + iOutGoingCnt + iMoldCnt == 0)
                {
                ...
                for (j = 0; j < 6; j++)
                {
                    if (j == 0)
                        ...
                        iCol = (int)OUTGOING.PROC_STATUS;
                    else if (j == 5)
                        iCol = (int)MOLD.PROC_STATUS;
                    ...
                    if (spdLotList.Sheets[j].RowCount > 0)
                    {
                        for (i = 0; i < spdLotList.Sheets[j].RowCount; i++)
                        {
                            ...
                            {
                                if(spdLotList.ActiveSheet != spdLotList.Sheets[j])
                                    spdLotList.ActiveSheet = spdLotList.Sheets[j];
                                ...
                                in_node.AddString("REMARKS", spdLotList.Sheets[j].Cells[i, (int)F_G.REMARKS].Value);
                                ...
                                else if(in_node.ProcStep == '2')
                                {
                                    ...
                                    in_node.AddString("FLOW", spdLotList.Sheets[j].Cells[i, (int)WIP.FLOW].Value);
                                    in_node.AddInt("FLOW_SEQ_NUM", spdLotList.Sheets[j].Cells[i, (int)WIP.FLOW_SEQ_NUM].Value);
                                }
                                ...
                                else if (in_node.ProcStep == '5')
                                {
                                    in_node.AddInt("LOT_SEQ", spdLotList.Sheets[j].Cells[i, (int)OUTGOING.LOT_SEQ].Value);
                                }
                                else if (in_node.ProcStep == '6')
                                {
                                    in_node.AddString("VENDOR_ID", spdLotList.Sheets[j].Cells[i, (int)MOLD.VENDOR].Value);
                                    in_node.AddString("VENDOR_SITE_ID", spdLotList.Sheets[j].Cells[i, (int)MOLD.VENDOR_SITE_ID].Value);
                                    in_node.AddString("VENDOR_LOT_ID", spdLotList.Sheets[j].Cells[i, (int)MOLD.VENDOR_LOT_ID].Value);
                                    in_node.AddInt("MOLD_MODEL", spdLotList.Sheets[j].Cells[i, (int)MOLD.MOLD_MODEL].Value);
                                    in_node.AddInt("MOLD_TYPE", spdLotList.Sheets[j].Cells[i, (int)MOLD.MOLD_TYPE].Value);
                                }
        ...
        private bool View_Raw_Material()
        {
            ...
            try
            {
                ...
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    spdLotList_Material.Cells[i, (int)INV.OPER_DESC].Value = dt.Rows[i]["OPER_DESC"];
                    spdLotList_Material.Cells[i, (int)INV.REMARKS].Value = dt.Rows[i]["REMARKS"];
                    spdLotList_Material.Cells[i, (int)INV.LEGACY_OPER].Value = dt.Rows[i]["WH_CODE"];
        ...
        private bool View_WIP()
        {
            ...
            try
            {
                ...
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    spdLotList_WIP.Cells[i, (int)WIP.OPER_DESC].Value = dt.Rows[i]["OPER_DESC"];
                    spdLotList_WIP.Cells[i, (int)WIP.REMARKS].Value = dt.Rows[i]["REMARKS"];
                    spdLotList_WIP.Cells[i, (int)WIP.LEGACY_OPER].Value = dt.Rows[i]["OP_CODE"];
                    spdLotList_WIP.Cells[i, (int)WIP.WO].Value = dt.Rows[i]["ORDER_ID"];
                    spdLotList_WIP.Cells[i, (int)WIP.FLOW].Value = dt.Rows[i]["FLOW"];
                    spdLotList_WIP.Cells[i, (int)WIP.FLOW_SEQ_NUM].Value = dt.Rows[i]["FLOW_SEQ_NUM"];
                }
        ...
        private bool View_Semi_Goods()
        {
            ...
            try
            {
                ...
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    spdLotList_Semi_Goods.Cells[i, (int)S_G.OPER_DESC].Value = dt.Rows[i]["OPER_DESC"];
                    spdLotList_Semi_Goods.Cells[i, (int)S_G.REMARKS].Value = dt.Rows[i]["REMARKS"];
        ...
        private bool View_Product()
        {
            ...
            try
            {
                ...
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    spdLotList_Finished_Goods.Cells[i, (int)F_G.OPER_DESC].Value = dt.Rows[i]["OPER_DESC"];
                    spdLotList_Finished_Goods.Cells[i, (int)F_G.REMARKS].Value = dt.Rows[i]["REMARKS"];
                    spdLotList_Finished_Goods.Cells[i, (int)F_G.LEGACY_OPER].Value = dt.Rows[i]["WH_CODE"];
                }
        ...
        private bool View_Outgoing()
        {
            ...
            try
            {
                ...
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    spdLotList_Outgoing.Cells[i, (int)OUTGOING.OPER_DESC].Value = dt.Rows[i]["OPER_DESC"];
                    spdLotList_Outgoing.Cells[i, (int)OUTGOING.LEGACY_OPER].Value = dt.Rows[i]["WH_CODE"];
                    spdLotList_Outgoing.Cells[i, (int)OUTGOING.LOT_SEQ].Value = dt.Rows[i]["LOT_SEQ"];
                }
        ...
        /// <summary>
        /// Legacy System의 금형 조회
        /// </summary>
        /// <returns></returns>
        private bool View_Mold()
        {
            TPDR.DirectViewCond[] dvcArgu = new TPDR.DirectViewCond[1];
            DataTable dt = null;
            string sSql = "";
            ...
            try
            {
                ...
                spdLotList_Mold.RowCount = 0;
                if (TPDR.GetDataOne("", ref dt, "CWIP2023-006", dvcArgu, false, false, ref sSql) == false)
                {
                    if (dt != null)
                        dt.Dispose();
                ...
                spdLotList_Mold.RowCount = dt.Rows.Count;
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    spdLotList_Mold.Cells[i, (int)MOLD.SEL].Value = false;
                    spdLotList_Mold.Cells[i, (int)MOLD.LOT_ID].Value = dt.Rows[i]["LOT_ID"];
                    spdLotList_Mold.Cells[i, (int)MOLD.MAT_ID].Value = dt.Rows[i]["MAT_ID"];
                    spdLotList_Mold.Cells[i, (int)MOLD.QTY].Value = dt.Rows[i]["LOT_QTY"];
                    spdLotList_Mold.Cells[i, (int)MOLD.OPER].Value = dt.Rows[i]["OPER"];
                    spdLotList_Mold.Cells[i, (int)MOLD.OPER_DESC].Value = dt.Rows[i]["OPER_DESC"];
                    spdLotList_Mold.Cells[i, (int)MOLD.REMARKS].Value = dt.Rows[i]["REMARKS"];
                    spdLotList_Mold.Cells[i, (int)MOLD.LEGACY_OPER].Value = dt.Rows[i]["WH_CODE"];
                    spdLotList_Mold.Cells[i, (int)MOLD.VENDOR].Value = dt.Rows[i]["VENDOR_ID"];
                    spdLotList_Mold.Cells[i, (int)MOLD.VENDOR_SITE_ID].Value = dt.Rows[i]["VENDOR_SITE_ID"];
                    spdLotList_Mold.Cells[i, (int)MOLD.VENDOR_LOT_ID].Value = dt.Rows[i]["VENDOR_LOT_ID"];
                    spdLotList_Mold.Cells[i, (int)MOLD.MOLD_MODEL].Value = dt.Rows[i]["MOLD_MODEL"];
                    spdLotList_Mold.Cells[i, (int)MOLD.MOLD_TYPE].Value = dt.Rows[i]["MOLD_TYPE"];
                }
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            spdLotList.ActiveSheet = spdLotList_Material;
        }
        ...
        private void frmTranMaterialMigration_Load(object sender, EventArgs e)
        {
            try
            {
                // 20.12.21 minjae.go
                MPCF.FitColumnHeader(spdLotList_Material);
                MPCF.FitColumnHeader(spdLotList_Mold);
                MPCF.FitColumnHeader(spdLotList_Outgoing);
                MPCF.FitColumnHeader(spdLotList_Semi_Goods);
                MPCF.FitColumnHeader(spdLotList_WIP);
                MPCF.FitColumnHeader(spdLotList_Finished_Goods);
                //
            }
            catch (Exception ex)
            {
