...
namespace CUS_WIP
{
    public partial class frmTranAreaItemMove : frmTranForm01
    {
        public frmTranAreaItemMove()
        {
            InitializeComponent();
            InitControl();
        }

        private void frmTranAreaItemMove_Load(object sender, EventArgs e)
        {
            string[] s_port;
            bool bSkip = false;
        ...
        */
        ...
        private void btnGCM_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                popup.StartPosition = FormStartPosition.CenterParent;
                popup.sTable_name = "C_AREA_MOVE_OPTION";

                if (MPGV.gcLanguage == '2')
                {
                    popup.sTable_name_desc = "사업부간 이동창고 등록";
                    popup.sComment = "해당 기준정보에 셋업된 창고로 이동이 진행됩니다.";

                }
                else
                {
                    popup.sTable_name_desc = "Move WareHosue Setup";
                    popup.sComment = "The move will be made to the warehouse set up for that Menu.";
                    ...
                    case CSGC.CHECK.ADD:
                        ...
                        for (int i = 0; i < svBeforeLot.RowCount; i++)
                        {
                            ...
                            if (Convert.ToBoolean(svBeforeLot.Cells[i, (int)BEFORE_LOT_INFO.CHECK].Value))
                            {
                                ...
                                string sBeforeError = svBeforeLot.Cells[i, (int)BEFORE_LOT_INFO.ERROR_CHECK].Value.ToString();

                                if (sBeforeError.ToString().Trim() == "")
                                {
                                    MPCF.ShowMsgBox(svBeforeLot.Cells[i, (int)BEFORE_LOT_INFO.ERROR_CHECK].Text);
        ...
        private void ViewLotList()
        {
            ...
            try
            {
                ...
                if (TPDR.GetDataOne("", ref dt, "CWIP2043-001", dvcItemList, false, false, ref sSql) == false)
                {
                    if (dt == null || dt.Rows.Count == 0)
                    {
                        //추후 메시지 필요하면 추가하는 것으로 현재 다른화면도 없음
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    svBeforeLot.Cells[i, (int)BEFORE_LOT_INFO.AFTER_AREA].Value = dt.Rows[i]["AFTER_AREA"];
                    svBeforeLot.Cells[i, (int)BEFORE_LOT_INFO.AFTER_MAT_ID].Value = dt.Rows[i]["AFTER_MAT_ID"];
                    svBeforeLot.Cells[i, (int)BEFORE_LOT_INFO.AFTER_MAT_DESC].Value = dt.Rows[i]["AFTER_MAT_DESC"];
                    svBeforeLot.Cells[i, (int)BEFORE_LOT_INFO.AFTER_OPER].Value = dt.Rows[i]["AFTER_OPER"];
                    svBeforeLot.Cells[i, (int)BEFORE_LOT_INFO.AFTER_OPER_DESC].Value = dt.Rows[i]["AFTER_OPER_DESC"];
                    svBeforeLot.Cells[i, (int)BEFORE_LOT_INFO.UNIT_PRICE].Value = dt.Rows[i]["UNIT_PRICE"];
                    svBeforeLot.Cells[i, (int)BEFORE_LOT_INFO.ERROR_CHECK].Value = dt.Rows[i]["ERROR_CHECK"];
            ...
            try
            {
                ...
                for (int i = svBeforeLot.RowCount - 1; i >= 0; i--)
                {
                    ...
                    if (Convert.ToBoolean(svBeforeLot.Cells[i, (int)BEFORE_LOT_INFO.CHECK].Value))
                    {
                        ...
                        svAfterLot.Cells[k, (int)AFTER_LOT_INFO.BEFORE_MAT_DESC].Value = svBeforeLot.GetValue(i, (int)BEFORE_LOT_INFO.MAT_DESC);
                        svAfterLot.Cells[k, (int)AFTER_LOT_INFO.AFTER_MAT_ID].Value = svBeforeLot.GetValue(i, (int)BEFORE_LOT_INFO.AFTER_MAT_ID);
                        svAfterLot.Cells[k, (int)AFTER_LOT_INFO.MAT_DESC].Value = svBeforeLot.GetValue(i, (int)BEFORE_LOT_INFO.AFTER_MAT_DESC);
                        ...
                        svAfterLot.Cells[k, (int)AFTER_LOT_INFO.AFTER_MAT_ID].Value = svBeforeLot.GetValue(i, (int)BEFORE_LOT_INFO.AFTER_MAT_ID);
                        svAfterLot.Cells[k, (int)AFTER_LOT_INFO.MAT_DESC].Value = svBeforeLot.GetValue(i, (int)BEFORE_LOT_INFO.MAT_DESC);

                        svAfterLot.Cells[k, (int)AFTER_LOT_INFO.AFTER_AREA].Value = svBeforeLot.GetValue(i, (int)BEFORE_LOT_INFO.AFTER_AREA);
                        svAfterLot.Cells[k, (int)AFTER_LOT_INFO.AFTER_OPER].Value = svBeforeLot.GetValue(i, (int)BEFORE_LOT_INFO.AFTER_OPER);
                        svAfterLot.Cells[k, (int)AFTER_LOT_INFO.UNIT_PRICE].Value = svBeforeLot.GetValue(i, (int)BEFORE_LOT_INFO.UNIT_PRICE);
                        svAfterLot.Cells[k, (int)AFTER_LOT_INFO.AFTER_OPER_DESC].Value = svBeforeLot.GetValue(i, (int)BEFORE_LOT_INFO.AFTER_OPER_DESC);
            ...
            try
            {
                ...
                for (int i = svAfterLot.RowCount - 1; i >= 0; i--)
                {
                    ...
                    if (Convert.ToBoolean(svAfterLot.Cells[i, (int)AFTER_LOT_INFO.CHECK].Value))
                    {
                        ...
                        svBeforeLot.Cells[k, (int)BEFORE_LOT_INFO.MAT_DESC].Value = svAfterLot.GetValue(i, (int)AFTER_LOT_INFO.BEFORE_MAT_DESC);
                        ...
                        svBeforeLot.Cells[k, (int)BEFORE_LOT_INFO.AFTER_MAT_ID].Value = svAfterLot.GetValue(i, (int)AFTER_LOT_INFO.AFTER_MAT_ID);
                        svBeforeLot.Cells[k, (int)BEFORE_LOT_INFO.AFTER_MAT_DESC].Value = svAfterLot.GetValue(i, (int)AFTER_LOT_INFO.MAT_DESC);
                        svBeforeLot.Cells[k, (int)BEFORE_LOT_INFO.AFTER_OPER].Value = svAfterLot.GetValue(i, (int)AFTER_LOT_INFO.AFTER_OPER);
                        svBeforeLot.Cells[k, (int)BEFORE_LOT_INFO.AFTER_OPER_DESC].Value = svAfterLot.GetValue(i, (int)AFTER_LOT_INFO.AFTER_OPER_DESC);
                        svBeforeLot.Cells[k, (int)BEFORE_LOT_INFO.UNIT_PRICE].Value = svAfterLot.GetValue(i, (int)AFTER_LOT_INFO.UNIT_PRICE);
            ...
            try
            {
                ...
                for (int i = 0; i < svAfterLot.RowCount; i++)
                {
                    ...
                    if(Convert.ToBoolean(svAfterLot.Cells[i, (int)AFTER_LOT_INFO.CHECK].Value))
                    {
                        ...
                        change_list.AddString("BEFORE_OPER", svAfterLot.Cells[i, (int)AFTER_LOT_INFO.OPER].Text);
                        ...
                        change_list.AddString("AFTER_AREA", svAfterLot.Cells[i, (int)AFTER_LOT_INFO.AFTER_AREA].Text);
                        change_list.AddString("AFTER_OPER", svAfterLot.Cells[i, (int)AFTER_LOT_INFO.AFTER_OPER].Text);
                        change_list.AddDouble("UNIT_PRICE", MPCF.ToDbl(svAfterLot.Cells[i, (int)AFTER_LOT_INFO.UNIT_PRICE].Text));
                        change_list.AddString("REQUESTS_IF_YN", "N");
                        change_list.AddString("RECEIPTS_IF_YN", "N");
        ...
        private void ViewLot(string as_lot_id)
        {
            ...
            try
            {
                ...
                if (TPDR.GetDataOne("", ref dt, "CWIP2043-003", dvcPrintType, false, false, ref sSql) == false)
                {
                    if (dt != null)
                        dt.Dispose();
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    spdAfterLotList.ActiveSheet.Cells[i+IRow, (int)AFTER_LOT_INFO.AFTER_MAT_ID].Value = dt.Rows[i]["AFTER_MAT_ID"];
                    spdAfterLotList.ActiveSheet.Cells[i+IRow, (int)AFTER_LOT_INFO.MAT_DESC].Value = dt.Rows[i]["AFTER_MAT_ID"];
                    ...
                    spdAfterLotList.ActiveSheet.Cells[i + IRow, (int)AFTER_LOT_INFO.AFTER_AREA].Value = dt.Rows[i]["AFTER_AREA"];
                    spdAfterLotList.ActiveSheet.Cells[i + IRow, (int)AFTER_LOT_INFO.AFTER_OPER].Value = dt.Rows[i]["AFTER_OPER"];
                    spdAfterLotList.ActiveSheet.Cells[i + IRow, (int)AFTER_LOT_INFO.AFTER_OPER_DESC].Value = dt.Rows[i]["AFTER_OPER_DESC"];
        ...
        private void frmTranAreaItemMove_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                spScanner.Close();
