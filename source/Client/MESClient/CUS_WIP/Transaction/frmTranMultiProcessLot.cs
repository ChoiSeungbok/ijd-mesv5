...
namespace CUS_WIP
{
    public partial class frmTranMultiProcessLot : CUS_COM.frmTranForm01
    {
        public frmTranMultiProcessLot()
        {
            InitializeComponent();
        ...
        private bool InputBomCheck()
        {
            try
            {
                ...
                in_node.ProcStep = '1';
                in_node.AddString("SQL", string.Format("SELECT * FROM MGCMTBLDAT WHERE FACTORY = '{0}' AND TABLE_NAME ='{1}' AND KEY_1 ='{2}'", MPGV.gsFactory, "C_BOM_CHK_OPER", cdvOper.Text));
        ...
        private void ViewLossList(string sLotID)
        {
            try
            {
                ...
                if (TPDR.GetDataOne("", ref dt, "CWIP2013-001", dvcArgu, false, false, ref sSql) == false)
                {
                    if (dt != null)
                        dt.Dispose();
        ...
        private void ViewWorkerList()
        {
            try
            {
                ...
                if (TPDR.GetDataOne("", ref dt, "CWIP2013-002", dvcArgu, false, false, ref sSql) == false)
                {
                    if (dt != null)
                        dt.Dispose();
        ...
        private void ViewToolList()
        {
            try
            {
                ...
                if (TPDR.GetDataOne("", ref dt, "CWIP2013-003", dvcArgu, false, false, ref sSql) == false)
                {
                    if (dt != null)
                        dt.Dispose();
        ...
        private void ViewStartLotList()
        {
            try
            {
                ...
                if (TPDR.GetDataOne("", ref dt, "CWIP2013-004", dvcArgu, false, false, ref sSql) == false)
                {
                    if (dt != null)
                        dt.Dispose();
                ...
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    spdLotList.ActiveSheet.Cells[i, (int)LOT_INFO.GOOD_QTY].Value = MPCF.ToDbl(dt.Rows[i]["QTY_1"]) - MPCF.ToDbl(dt.Rows[i]["LOSS_QTY"]);
                    spdLotList.ActiveSheet.Cells[i, (int)LOT_INFO.BAD_QTY].Value = dt.Rows[i]["LOSS_QTY"];
                    spdLotList.ActiveSheet.Cells[i, (int)LOT_INFO.START_TIME].Value = MPCF.MakeDateFormat(dt.Rows[i]["START_TIME"].ToString(), DATE_TIME_FORMAT.DATETIME);
                    spdLotList.ActiveSheet.Cells[i, (int)LOT_INFO.IN_QTY].Value = dt.Rows[i]["QTY_1"];
                }
                MPCF.FitColumnHeader(spdLotList);
        ...
        private bool SaveProcess(char c_step)
        {
            ...
            try
            {
                ...
                if (MPCR.CallService("CUS_WIP", "CUS_WIP_Multi_Process_Lot", in_node, ref out_node) == false)
                    return false;
                else
                    MPCR.ShowSuccessMsg(out_node);
                ...
                if (c_step == '1')
                {
                    ...
                    if (CheckPrintOption(cdvOper.Text, sOrder_id))
                    {
                        if (sPrintStartYN == "Y")
                            bPrint = true;
                    }
        ...
        private void frmTranMultiProcessLot_Load(object sender, EventArgs e)
        {
            string[] s_port;
            bool bSkip = false;
            ...
            try
            {
                ...
                if(g_PopYn == "Y")
                {
                    if (MPCF.Trim(g_Lot_ID) == "")
                        return;


                    txtLotID.Text = g_Lot_ID;
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                spdLotList.ActiveSheet.SetValue(iRow, (int)LOT_INFO.CHK, true);
                spdLotList.ActiveSheet.SetValue(iRow, (int)LOT_INFO.ORDER_ID, cdvOrder.Text);
                spdLotList.ActiveSheet.SetValue(iRow, (int)LOT_INFO.LOT_ID, "");
                spdLotList.ActiveSheet.SetValue(iRow, (int)LOT_INFO.MAT_ID, "");
                spdLotList.ActiveSheet.SetValue(iRow, (int)LOT_INFO.MAT_DESC, "");
        ...
        private void btnMatSetup_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (spdLotList_Sheet1.Rows.Count == 0)
                {
                    return;
                }

                if (spdLotList_Sheet1.ActiveRowIndex < 0)
                {
                    return;
                }
                ...
                popup.s_MatId = spdLotList_Sheet1.Cells[0, (int)LOT_INFO.MAT_ID].Text;
                popup.s_MatDesc = spdLotList_Sheet1.Cells[0, (int)LOT_INFO.MAT_DESC].Text;
                popup.i_MatVer = 1;
                /*
