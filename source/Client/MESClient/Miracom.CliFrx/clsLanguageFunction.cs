...
namespace Miracom.CliFrx
{
    public class clsLanguageFunction
    {
        ...
        public bool ConvertMessage(System.Windows.Forms.Control.ControlCollection colControl)
        {
            ...
            try
            {
                ...
                foreach (Control tempLoopVar_l_Control in colControl)
                {
                    ...
                    //ultraButton 캡션 변경 추가. _20200723_lwg
                    else if (l_Control is Infragistics.Win.Misc.UltraButton)
                    {
                        ((Infragistics.Win.Misc.UltraButton)l_Control).Text = FindLanguage(((Infragistics.Win.Misc.UltraButton)l_Control).Text, 1);
                    }
                    //UltraGroupBox 캡션 변경 추가. _20200723_lwg
                    else if (l_Control is Infragistics.Win.Misc.UltraGroupBox)
                    {
                        ((Infragistics.Win.Misc.UltraGroupBox)l_Control).Text = FindLanguage(((Infragistics.Win.Misc.UltraGroupBox)l_Control).Text, 0);
                    ...
                    else if (l_Control is FpSpread)
                    {
                        if (((FpSpread)l_Control).Tag != null)
                        {
                            ...
                            {
                                for (k = 0; k < ((FpSpread)l_Control).Sheets.Count; k++)
                                {
                                    s_text = ((FpSpread)l_Control).Sheets[k].SheetName;
                                    if (s_text != "")
                                        s_text = ((FpSpread)l_Control).Sheets[k].SheetName = FindLanguage(s_text, 0);

                                    for (j = 0; j < ((FpSpread)l_Control).Sheets[k].RowCount; j++)
                                    {
                                        for (i = 0; i < ((FpSpread)l_Control).Sheets[k].ColumnCount; i++)
                                        {
                                            s_text = MPCF.Trim(((FpSpread)l_Control).Sheets[k].Cells[j, i].Value);
                                            if (s_text != "")
                                            {
                                                ((FpSpread)l_Control).Sheets[k].Cells[j, i].Value = FindLanguage(s_text, 0);
                                            }
                                        }
                                    }
                                }
                            ...
                            else
                            {
                                for (k = 0; k < ((FpSpread)l_Control).Sheets.Count; k++)
                                {
                                    ...
                                    for (j = 0; j < ((FpSpread)l_Control).Sheets[k].ColumnHeader.RowCount; j++)
                                    {
                                        for (i = 0; i < ((FpSpread)l_Control).Sheets[k].ColumnHeader.Columns.Count; i++)
                                        {
                                            s_text = MPCF.Trim(((FpSpread)l_Control).Sheets[k].ColumnHeader.Cells[j, i].Value);
                                            if (s_text != "")
                                            {
                                                ((FpSpread)l_Control).Sheets[k].ColumnHeader.Cells[j, i].Value = FindLanguage(s_text, 0);
                                            }
                                        }
                                    }
                                    for (j = 0; j < ((FpSpread)l_Control).Sheets[k].RowHeader.Rows.Count; j++)
                                    {
                                        for (i = 0; i < ((FpSpread)l_Control).Sheets[k].RowHeader.ColumnCount; i++)
                                        {
                                            s_text = MPCF.Trim(((FpSpread)l_Control).Sheets[k].RowHeader.Cells[j, i].Value);
                                            if (s_text != "")
                                            {
                                                ((FpSpread)l_Control).Sheets[k].RowHeader.Cells[j, i].Value = FindLanguage(s_text, 0);
                                            }
                                        }
                                    }
                                }
        ...
        public string FindLanguage(string S, CAPTION_TYPE captionType)
        {
            return FindLanguage(S, (int)captionType);
        }
        public string FindLanguage(string S, int iType)
        {
        ...
        public string GetMessage(int i)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        if (MPGV.gsMessageData[1, i] == null)
                        {
                            returnValue = "No Error Message";
                        }
                        else
                        {
                            returnValue = MPGV.gsMessageData[1, i];
                        }
                        
                    }
                    else if (MPGV.gcLanguage == '3')
                    {
                        if (MPGV.gsMessageData[2, i] == null)
                        {
                            returnValue = "No Error Message";
                        }
                        else
                        {
                            returnValue = MPGV.gsMessageData[2, i];
                        }
                        
                    }
                    else
                    {
                        if (MPGV.gsMessageData[0, i] == null)
                        {
                            returnValue = "No Error Message";
                        }
                        else
                        {
                            returnValue = MPGV.gsMessageData[0, i];
                        }
                    }
                }
                return returnValue;
        ...
        public bool LoadMessageResource(string sMessageFile)
        {
            ...
            int i;
            int j;
            MPGV.gsMessageData = new string[3, MPGV.giMaxMessageData];
            try
            {
                        ...
                        case XmlNodeType.Element:
                            ...
                            switch (xtr.Name.ToUpper())
                            {
                                case "TEXT":
                                    ...
                                    MPGV.gsMessageData[0, j] = sTemp[0];
                                    MPGV.gsMessageData[1, j] = sTemp[1];
                                    MPGV.gsMessageData[2, j] = sTemp[2];
                                    break;
                            }
                            break;
