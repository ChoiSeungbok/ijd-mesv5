...
{
    public partial class frmWIPTranMultiEndLot : Miracom.MESCore.TranForm08
    {
        public frmWIPTranMultiEndLot()
        {
            InitializeComponent();
        }
        ...
        private bool CheckCondition(string FuncName)
        {
            ...
            switch (MPCF.Trim(FuncName))
            {
                case "VIEW":
                    ...
                    {
                        if (MPCF.CheckValue(cdvStartResID, 1) == false)
                        {
        ...
        private bool ViewLotList()
        {
            ...
            in_node.AddString("RES_ID", MPCF.Trim(cdvStartResID.Text));
            in_node.AddChar("ZERO_QTY_FLAG", chkZeroQuantityLot.Checked ? 'Y' : ' ');
            in_node.AddChar("START_END_FLAG", chkOnlyStartLot.Checked ? 'S' : ' ');
        ...
        //
        private bool EndLot(char ProcStep, int iRowIndex, TRSNode out_node)
        {
            bool b_have_port;
        ...
        private void frmWIPTranMultiEndLot_Load(object sender, System.EventArgs e)
        {
        ...
        private void frmWIPTranMultiEndLot_Activated(object sender, System.EventArgs e)
        {
        ...
        private void cdvStartResID_ButtonPress(object sender, System.EventArgs e)
        {
            ...
            if (MPCF.Trim(cdvOperation.Text) == "")
            {
                cdvStartResID.RefuseEventExec = true;
            ...
            cdvStartResID.ListCond_Step = '2';
            cdvStartResID.ListCond_MatID = MPCF.Trim(cdvMaterial.Text);
            cdvStartResID.ListCond_MatVersion = cdvMaterial.Version;
            cdvStartResID.ListCond_Flow = MPCF.Trim(cdvFlow.Text);
            cdvStartResID.ListCond_Oper = MPCF.Trim(cdvOperation.Text);
            cdvStartResID.ListCond_IncludeDeleteResource = false;
        }
        ...
        /* 2013.06.12. Aiden. Lot 가 동일한 장비에서 작업중인지 확인하는 기능 추가 */
        private void lisLotList_ItemChecked(object sender, ItemCheckedEventArgs e)
        {
            ...
            for (i = 0; i < lisLotList.Items.Count; i++)
            {
                ...
                {   
                    ...
                    {
                        cdvResID.Text = lisLotList.Items[i].SubItems[47].Text;
                    }

                    if (MPCF.Trim(cdvResID.Text) != lisLotList.Items[i].SubItems[47].Text)
                    {
                        lisLotList.Items[i].Checked = false;

                        MPCF.ShowMsgBox(MPCF.GetMessage(948));
                    }
