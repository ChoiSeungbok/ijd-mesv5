...
//
//   System      : MES
//   File Name   : frmWIPTranQAMaint.vb
...
{
	public partial class frmWIPTranQAMaint: Miracom.MESCore.TranForm07
	{
		
		public frmWIPTranQAMaint()
		{
			InitializeComponent();
		}
        ...
        // ViewQAHistoryByLot()
		//       - View Qa History By Lot 
		// Return Value
        //       - Boolean : True or False
		// Arguments
        //        ByVal Lot_ID, Oper As String : Lot_Id, Oper
        //        ByVal Hist_Seq, Qa_Hist_Seq As int : Hist_Seq, Qa_Hist_Seq
        private bool ViewQAHistoryByLot(string sLotID, string sOper, int iHistSeq, int iQAHistSeq)
        {
            TRSNode in_node = new TRSNode("VIEW_QA_HIST_BY_LOT_IN");
            TRSNode out_node = new TRSNode("VIEW_QA_HIST_BY_LOT_OUT");
			...
			try
			{
                ...
                in_node.AddInt("QA_HIST_SEQ", iQAHistSeq);
                in_node.AddString("OPER", sOper);

                if (MPCR.CallService("WIP", "WIP_View_QA_History_By_Lot", in_node, ref out_node) == false)
                {
                    return false;
                }

                s_Temp = MPCF.Trim(out_node.GetString("TRAN_TIME"));
                dtpHisDate.Value = MPCF.ToDate(s_Temp);
                dtpHisTime.Value = MPCF.ToDate(s_Temp);
                ...
                txtInspector.Text = MPCF.Trim(out_node.GetString("INSPECTOR"));
                cdvShift.Text = MPCF.Trim(out_node.GetString("SHIFT"));
                txtIRRMRR.Text = MPCF.Trim(out_node.GetString("IRRMRR"));
                txtComment.Text = MPCF.Trim(out_node.GetString("QA_COMMENT"));
                cdvResID.Text = MPCF.Trim(out_node.GetString("RES_ID"));

                cdvCMF1.Text = MPCF.Trim(out_node.GetString("QA_CMF_1"));
                cdvCMF2.Text = MPCF.Trim(out_node.GetString("QA_CMF_2"));
                cdvCMF3.Text = MPCF.Trim(out_node.GetString("QA_CMF_3"));
                cdvCMF4.Text = MPCF.Trim(out_node.GetString("QA_CMF_4"));
                cdvCMF5.Text = MPCF.Trim(out_node.GetString("QA_CMF_5"));
                cdvCMF6.Text = MPCF.Trim(out_node.GetString("QA_CMF_6"));
                cdvCMF7.Text = MPCF.Trim(out_node.GetString("QA_CMF_7"));
                cdvCMF8.Text = MPCF.Trim(out_node.GetString("QA_CMF_8"));
                cdvCMF9.Text = MPCF.Trim(out_node.GetString("QA_CMF_9"));
                cdvCMF10.Text = MPCF.Trim(out_node.GetString("QA_CMF_10"));

                cdvCMF11.Text = MPCF.Trim(out_node.GetString("RESV_FIELD_1"));
                cdvCMF12.Text = MPCF.Trim(out_node.GetString("RESV_FIELD_2"));
                cdvCMF13.Text = MPCF.Trim(out_node.GetString("RESV_FIELD_3"));
                cdvCMF14.Text = MPCF.Trim(out_node.GetString("RESV_FIELD_4"));
                cdvCMF15.Text = MPCF.Trim(out_node.GetString("RESV_FIELD_5"));
                cdvCMF16.Text = MPCF.Trim(out_node.GetString("RESV_FIELD_6"));
                cdvCMF17.Text = MPCF.Trim(out_node.GetString("RESV_FIELD_7"));
                cdvCMF18.Text = MPCF.Trim(out_node.GetString("RESV_FIELD_8"));
                cdvCMF19.Text = MPCF.Trim(out_node.GetString("RESV_FIELD_9"));
                cdvCMF20.Text = MPCF.Trim(out_node.GetString("RESV_FIELD_10"));
                
                cdvAction.Text = MPCF.Trim(out_node.GetString("ACT_RULE_ID"));
                ...
                for (i = 0; i < out_node.GetInt("LOTDEF_COUNT"); i++)
                {
                    out_node.GetList("LOTDEF_TBL");
                    if (out_node.GetList("LOTDEF_TBL")[i_Unit1Def].GetInt("DEF_SEQ") == 1)
                    {
                        if (i_Unit1Def == 0)
                        {
                            cdvUnit1Code1.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit1Qty1.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit1Def == 1)
                        {
                            cdvUnit1Code2.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit1Qty2.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit1Def == 2)
                        {
                            cdvUnit1Code3.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit1Qty3.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit1Def == 3)
                        {
                            cdvUnit1Code4.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit1Qty4.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit1Def == 4)
                        {
                            cdvUnit1Code5.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit1Qty5.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit1Def == 5)
                        {
                            cdvUnit1Code6.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit1Qty6.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit1Def == 6)
                        {
                            cdvUnit1Code7.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit1Qty7.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit1Def == 7)
                        {
                            cdvUnit1Code8.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit1Qty8.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit1Def == 8)
                        {
                            cdvUnit1Code9.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit1Qty9.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit1Def == 9)
                        {
                            cdvUnit1Code10.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit1Qty10.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        i_Unit1Def++;
                    }
                    else if (out_node.GetList("LOTDEF_TBL")[i].GetInt("DEF_SEQ") == 1)
                    {
                        if (i_Unit2Def == 0)
                        {
                            cdvUnit2Code1.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit2Qty1.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit2Def == 1)
                        {
                            cdvUnit2Code2.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit2Qty2.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit2Def == 2)
                        {
                            cdvUnit2Code3.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit2Qty3.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit2Def == 3)
                        {
                            cdvUnit2Code4.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit2Qty4.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit2Def == 4)
                        {
                            cdvUnit2Code5.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit2Qty5.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit2Def == 5)
                        {
                            cdvUnit2Code6.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit2Qty6.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit2Def == 6)
                        {
                            cdvUnit2Code7.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit2Qty7.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit2Def == 7)
                        {
                            cdvUnit2Code8.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit2Qty8.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_QTY").ToString();
                        }
                        else if (i_Unit2Def == 8)
                        {
                            cdvUnit2Code9.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit2Qty9.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        else if (i_Unit2Def == 9)
                        {
                            cdvUnit2Code10.Text = out_node.GetList("LOTDEF_TBL")[i].GetString("DEF_CODE");
                            txtUnit2Qty10.Text = out_node.GetList("LOTDEF_TBL")[i].GetDouble("DEF_QTY").ToString();
                        }
                        i_Unit2Def++;
                    }
                }
                    ...
                    )
                {
                    for (i = 0; i < out_node.GetInt("SUBLOT_COUNT"); i++)
                    {
                        ...
                        for (j = 0; j < lisSubLot.Items.Count; j++)
                        {
                            if (out_node.GetList("SUBLOT_TBL")[i].GetInt("SLOT_NO").ToString() == lisSubLot.Items[j].SubItems[1].Text)
                            {
                                ...
                                lisSubLot.Items[j].SubItems[3].Text = out_node.GetList("SUBLOT_TBL")[i].GetDouble("DEF_QTY").ToString();
                                lisSubLot.Items[j].SubItems[4].Text = out_node.GetList("SUBLOT_TBL")[i].GetDouble("GOOD_QTY").ToString();
                                lisSubLot.Items[j].SubItems[5].Text = out_node.GetList("SUBLOT_TBL")[i].GetDouble("YIELD").ToString();
                            }
                        }
                    ...
                    for (i = 0; i < out_node.GetInt("SUBLOT_DEF_COUNT"); i++)
                    {
                        out_node.GetList("SUBLOT_DEF_TBL");

                        spdDefectData_LotInfo.SetValue(i, 0, out_node.GetList("SUBLOT_DEF_TBL")[i].GetString("DEF_CODE"));
                        spdDefectData_LotInfo.SetValue(i, 2, out_node.GetList("SUBLOT_DEF_TBL")[i].GetString("SUBLOT_ID"));
                        spdDefectData_LotInfo.SetValue(i, 5, out_node.GetList("SUBLOT_DEF_TBL")[i].GetDouble("DEF_QTY").ToString());

                    }
                    spdDefectData_LotInfo.ActiveColumnIndex = 5;
                    spdDefectData_LotInfo.ActiveRowIndex = 0;
                    spdDefectData_EditModeOff(null, null);
                    chkWaferSelect.Checked = true;
                    chkWaferSelect_CheckedChanged(null, null);
        ...
        //       
        // Arguments
        //          ByVal Unit1, Unit2 As bool : Unit1, Unit2
		...
		private bool QA_Gate(char ProcStep)
		{
            ...
            try
            {
                ...
                {
                    in_node.AddDouble("DEF_QTY_1", MPCF.ToDbl(txtSMPSize1.Text) - MPCF.ToDbl(txtOutQty1.Text));
                    in_node.AddDouble("YIELD_1", (MPCF.ToDbl(txtOutQty1.Text) / MPCF.ToDbl(txtSMPSize1.Text)) * 100);
                }
                if (MPCF.ToDbl(txtSMPSize2.Text) > 0)
                {
                    in_node.AddDouble("DEF_QTY_2", MPCF.ToDbl(txtSMPSize2.Text) - MPCF.ToDbl(txtOutQty2.Text));
                    in_node.AddDouble("YIELD_2", (MPCF.ToDbl(txtOutQty2.Text) / MPCF.ToDbl(txtSMPSize2.Text)) * 100);
                }
                ...
                in_node.AddString("TRAN_TIME", dtpHisDate.Value.ToString("yyyyMMdd") + dtpHisTime.Value.ToString("HHmmss"));
                in_node.AddInt("HIST_SEQ", MPCF.ToInt(cdvHistSeq.Text));
                in_node.AddInt("QA_HIST_SEQ", MPCF.ToInt( cdvQAHistSeq.Text));
                ...
                if (MPCR.CallService("WIP", "WIP_QA_Maint", in_node, ref out_node) == false)
                {
                    return false;
                }
                return true;
        ...
        private void frmWIPTranQAMaint_Activated(object sender, System.EventArgs e)
        {
            try
            {
                if (b_load_flag == false)
                {
                    ...
                    spdDefectData_LotInfo.RowCount = 30;
                    ...
                    spdDefectData.Enabled = false;
                    SetLocCellField();
                    txtTranDateTime.Visible = true;
                    chkTranDateTime.Visible = true;
                    dtpTranDate.Visible = true;
                    dtpTranTime.Visible = true;
                    dtpHisDate.Value = System.DateTime.Now;
                    dtpTranTime.Value = System.DateTime.Now;
		...
		private void btnProcess_Click(System.Object sender, System.EventArgs e)
		{
			try
			{
				...
				if (QA_Gate((MPGC.MP_STEP_UPDATE)) == false) return;
        ...
        private void spdDefectData_EditModeOff(object sender, EventArgs e)
        {
            ...
            if(spdDefectData_LotInfo.ActiveCell.Column.Index!=5)
                return;
        ...
        private void cdvOper_ButtonPress(object sender, EventArgs e)
        {
            ...
            sTemp = "SELECT OPR.OPER, OPR.OPER_DESC FROM (SELECT OPER FROM MQCGLOTHIS WHERE LOT_ID='" + txtLotID.Text + "' AND HIST_DEL_FLAG=' ' GROUP BY OPER) HIS, MWIPOPRDEF OPR WHERE";
            sTemp = sTemp + " HIS.OPER = OPR.OPER ORDER BY OPR.OPER";

            if ( BASLIST.ViewQueryList(cdvOper.GetListView, '1',sTemp, 0 , null) == false)
                return;        
        }

        private void cdvHistSeq_ButtonPress(object sender, EventArgs e)
        {
            string sTemp;
            cdvHistSeq.Init();
            cdvHistSeq.Columns.Add("Hist Seq", 50, HorizontalAlignment.Left);
            cdvHistSeq.Columns.Add("Trans Time", 100, HorizontalAlignment.Left);
            cdvHistSeq.SelectedSubItemIndex = 0;

            sTemp = "SELECT HIST_SEQ, MAX(TRAN_TIME) FROM MQCGLOTHIS WHERE LOT_ID='" + txtLotID.Text + "' AND OPER='" + cdvOper.Text + "' AND HIST_DEL_FLAG=' ' GROUP BY HIST_SEQ ORDER BY HIST_SEQ";

            if (BASLIST.ViewQueryList(cdvHistSeq.GetListView, '1', sTemp, 0, null) == false)
                return;  
        }

        private void cdvQAHistSeq_ButtonPress(object sender, EventArgs e)
        {
            string sTemp;
            cdvQAHistSeq.Init();
            cdvQAHistSeq.Columns.Add("QA Hist Seq", 50, HorizontalAlignment.Left);
            cdvQAHistSeq.Columns.Add("Trans Time", 100, HorizontalAlignment.Left);
            cdvQAHistSeq.SelectedSubItemIndex = 0;

            sTemp = "SELECT QA_HIST_SEQ, TRAN_TIME FROM MQCGLOTHIS WHERE LOT_ID='" + txtLotID.Text + "' AND OPER='" + cdvOper.Text + "' AND HIST_SEQ=" + cdvHistSeq.Text + " AND HIST_DEL_FLAG=' ' ORDER BY QA_HIST_SEQ";

            if (BASLIST.ViewQueryList(cdvQAHistSeq.GetListView, '1', sTemp, 0, null) == false)
                return;  
        }
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            lisHistory.Columns.Clear();
            lisHistory.Columns.Add("History Seq", 50, HorizontalAlignment.Left);
            lisHistory.Columns.Add("QA History Seq", 50, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Trans Time", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Factory", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Material ID", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Material Version", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Flow", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Flow Seq Num", 50, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Operation", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Resource", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Carrier", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Qty1", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Qty2", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Qty3", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Sample Rule", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Action Rule", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Pass Flag", 50, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Sample Size1", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Sample Size2", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Defect Qty1", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Defect Qty2", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Yield1", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Yield2", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Test Type", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Inspector", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Shift", 50, HorizontalAlignment.Left);
            lisHistory.Columns.Add("IRR/MRR", 50, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Comment", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Alram Code", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Hist Delete Flag", 50, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Hist Del Time", 50, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Hist Del User", 50, HorizontalAlignment.Left);
            lisHistory.Columns.Add("Hist Del Comment", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("CMF1", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("CMF2", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("CMF3", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("CMF4", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("CMF5", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("CMF6", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("CMF7", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("CMF8", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("CMF9", 100, HorizontalAlignment.Left);
            lisHistory.Columns.Add("CMF10", 100, HorizontalAlignment.Left);

            sTemp = "SELECT HIST_SEQ, QA_HIST_SEQ, TRAN_TIME, FACTORY, MAT_ID, MAT_VER, FLOW, FLOW_SEQ_NUM, OPER, RES_ID,";
            sTemp = sTemp + " CRR_ID, QTY_1, QTY_2, QTY_3, SMP_RULE_ID, ACT_RULE_ID, PASS_FLAG, SMP_SIZE_1, ";
            sTemp = sTemp + " SMP_SIZE_2, DEF_QTY_1, DEF_QTY_2, YIELD_1, YIELD_2, TEST_TYPE, INSPECTOR, SHIFT, IRRMRR, ";
            sTemp = sTemp + " QA_COMMENT, ALARM_CODE, HIST_DEL_FLAG, HIST_DEL_TIME, HIST_DEL_USER, HIST_DEL_COMMENT, ";
            sTemp = sTemp + " QA_CMF_1, QA_CMF_2, QA_CMF_3, QA_CMF_4, QA_CMF_5, QA_CMF_6, QA_CMF_7, QA_CMF_8, QA_CMF_9, QA_CMF_10";
            sTemp = sTemp + " FROM MQCGLOTHIS WHERE LOT_ID='" + txtLotID.Text + "'";
            sTemp = sTemp + " AND OPER='"+ cdvOper.Text + "'";

            if(MPCF.Trim(cdvHistSeq.Text)!="")
                sTemp = sTemp + " AND HIST_SEQ=" + cdvHistSeq.Text;

            if(MPCF.Trim(cdvQAHistSeq.Text)!="")
                sTemp = sTemp + " AND QA_HIST_SEQ=" + cdvQAHistSeq.Text;

            sTemp = sTemp + " AND HIST_DEL_FLAG=' ' ORDER BY HIST_SEQ, QA_HIST_SEQ";

            if (BASLIST.ViewQueryList(lisHistory, '1', sTemp, 0, null) == false)
                return;
        }

        private void lisHistory_DoubleClick(object sender, EventArgs e)
        {
            if (lisHistory.SelectedItems.Count > 0)
            {
                if (ViewQAHistoryByLot(txtLotID.Text, cdvOper.Text, MPCF.ToInt(lisHistory.SelectedItems[0].Text), MPCF.ToInt(lisHistory.SelectedItems[0].SubItems[1].Text)) == false)
                {
                    return;
                }
            }
        ...
        private void lisHistory_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (lisHistory.SelectedItems.Count > 0)
            {
                cdvHistSeq.Text = lisHistory.SelectedItems[0].Text;
                cdvQAHistSeq.Text = lisHistory.SelectedItems[0].SubItems[1].Text;
                if (ViewQAHistoryByLot(txtLotID.Text, cdvOper.Text, MPCF.ToInt(lisHistory.SelectedItems[0].Text), MPCF.ToInt(lisHistory.SelectedItems[0].SubItems[1].Text)) == false)
                {
                    return;
                }
        ...
        {
            MPCF.InitListView(lisHistory);
            this.ViewLotInfo(txtLotID.Text);
        }
    }
