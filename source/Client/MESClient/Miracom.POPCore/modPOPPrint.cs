    ...
    public sealed class modPOPPrint
    {
            ...
            try
            {
                ...
                switch (sPrintType.Substring(0, 3))
                {
                    case MPGC.POP_PRINT_PORT_LPT:
                        ...
                        /* LPT 포트가 활성화 되어 Status 가 OK가 아니면 IsInvalid가 true가 됨. */
                        if (hFile.IsInvalid)
                        {
                            return false;
                        }
        ...
        public static bool ComPortOpen(ref Rs232 aComPort, int aPort, int iBaudRate, int iParity, int iDataBit, int iStopBit)
        {
            ...
            try
            {
                ...
                with_1.Timeout = 1500; //Timeout 설정
        ...
        public static bool ExcutePrint(string sPrintType, Rs232 aComPort, ref TRSNode out_node, string sPrintModel) //
        {
            ...
            try
            {
                ...
                {
                    //내부에서 Byte 치환 하여 sending to print 처리
        ...
        public static bool PrtStatusCheck(string sPrintType, ref Rs232 aComPort)
        {
            ...
            if (sPrintType.Substring(0, 3) == MPGC.POP_PRINT_PORT_COM)
            {
                //Port로 부터 Message Read 중 Timeout 발생하여, 임시 주석처리함(20121027 lakekim)
                return true;
                ...
                // 프린터상태값이 모두 들어올때까지기다림.( TimeOut 5Sec)
                do
                {
                ...
                if (b_TimeOut) // 吏?곗떆媛?珥덇낵??
                {
                    Console.Beep();
                    MessageBox.Show("프린터가 꺼져 있거나 통신 케이블이 빠진것 같습니다.");
                ...
                else
                {
                    ...
                    {
                        Console.Beep();
                        s_PrtErrMsg = s_PrtErrMsg + "프린터에 용지(Label)이 없습니다." + (char)MPGC.CR;
                    }

                    if (MPGC.s_PrtStatus.Substring(7, 1) == "1")
                    {
                        Console.Beep();
                        s_PrtErrMsg = s_PrtErrMsg + "중지램프(Pause-lamp)가 들어와 있습니다." + (char)MPGC.CR;
                    }
                    ...
                    {
                        Console.Beep();
                        s_PrtErrMsg = s_PrtErrMsg + "프린터 버퍼(Buffer)가 가득차 있습니다." + (char)MPGC.CR;
                    }
                    if (MPGC.s_PrtStatus.Substring(41, 1) == "1")
                    {
                        Console.Beep();
                        s_PrtErrMsg = s_PrtErrMsg + "프린터 헤드(Head)가 올려져 있습니다." + (char)MPGC.CR;
                    }
                    if (MPGC.s_PrtStatus.Substring(43, 1) == "1")
                    {
                        Console.Beep();
                        s_PrtErrMsg = s_PrtErrMsg + "리본(Ribbon)이 없습니다." + (char)MPGC.CR;
                    }
