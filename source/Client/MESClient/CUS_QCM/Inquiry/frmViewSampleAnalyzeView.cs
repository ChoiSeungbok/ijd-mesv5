...
namespace CUS_QCM
{
    public partial class frmViewSampleAnalyzeView : CUS_COM.frmViewForm01
    {
        public frmViewSampleAnalyzeView()
        {
            InitializeComponent();
        ...
        private void ViewOrderList()
        {
            try
            {
                ...
                dvcArgu[9].sCondition_ID = "ANALYZE_STATUS";
                dvcArgu[9].sCondition_Value = "%" + cdvAnalyzeStatus.Text + "%";



                dvcArgu[10].sCondition_ID = "GRADE";
                dvcArgu[10].sCondition_Value = "%" + cdvGrade.Text + "%";

                dvcArgu[11].sCondition_ID = "CMF_01";
                dvcArgu[11].sCondition_Value = "%" + cdvGrade2.Text + "%";


                sViewID = "CQCM3017-002";
                ...
                {
                    spdSampleList_Sheet1.RowCount++;

                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.REPORT_NO].Value = dt.Rows[i]["REPORT_NO"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.CMF_02].Value = dt.Rows[i]["CMF_02"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.CUST_NAME].Value = dt.Rows[i]["CUST_NAME"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.EVENT_DATE].Value = dt.Rows[i]["EVENT_DATE"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.RECEIVED_DATE_1].Value = dt.Rows[i]["RECEIVED_DATE_1"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.RECEIVED_DATE_2].Value = dt.Rows[i]["RECEIVED_DATE_2"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.GRADE].Value = dt.Rows[i]["GRADE"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.CMF_01].Value = dt.Rows[i]["CMF_01"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.CLOSURE_DATE].Value = dt.Rows[i]["CLOSURE_DATE"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.SHARE_DATE].Value = dt.Rows[i]["SHARE_DATE"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.SAL_GROUP].Value = dt.Rows[i]["SAL_GROUP"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.SAL_USER].Value = dt.Rows[i]["SAL_USER"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.ANALYZE_STATUS].Value = dt.Rows[i]["ANALYZE_STATUS"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.COMMENT_01].Value = dt.Rows[i]["COMMENT_01"];
                    spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.COMMENT_02].Value = dt.Rows[i]["COMMENT_02"];



                    if (dt.Rows[i]["FILE_NO_01"].ToString().Trim() != "")
                    {
                        spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.FILE_NO_01].CellType = buttonCellType1;
                        spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.FILE_NO_01].Tag = dt.Rows[i]["FILE_NO_01"];

                    }
                    else
                    {
                        spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.FILE_NO_01].CellType = buttonCellType2;
                    }

                    if (dt.Rows[i]["FILE_NO_02"].ToString().Trim() != "")
                    {
                        spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.FILE_NO_02].CellType = buttonCellType1;
                        spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.FILE_NO_02].Tag = dt.Rows[i]["FILE_NO_02"];
                    }
                    else
                    {
                        spdSampleList_Sheet1.Cells[i, (int)ANALYZE_LIST.FILE_NO_02].CellType = buttonCellType2;
                    }

                }
                MPCF.FitColumnHeader(spdSampleList);
        ...
        private void frmViewSampleAnalyzeView_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void spdOrderList_CellClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            try
            {
                ...
                {
                    if (spdSampleList.ActiveSheet.ColumnHeader.Cells[e.Row, e.Column].CellType is FarPoint.Win.Spread.CellType.CheckBoxCellType)
                    {
                        CSCF.CheckSpreadCell(spdSampleList, e.Row, e.Column, true, true, false);
                    }
                }
                if (e.ColumnHeader == false && e.ColumnFooter == false)
                {
                    EXTFUC.SetSelectRowColor(spdSampleList, e.Row);
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            try
            {
                EXTFUC.SaveExcel(spdSampleList, "PackOrderList");
        ...
        private void spdSampleList_CellDoubleClick(object sender, CellClickEventArgs e)
        {
            try
            {
                ...
                if (spdSampleList_Sheet1.ActiveColumnIndex != (int)ANALYZE_LIST.FILE_NO_01 && spdSampleList_Sheet1.ActiveColumnIndex != (int)ANALYZE_LIST.FILE_NO_02)
                {
                    frmTranSampleanalyzeRegistraion popup = new frmTranSampleanalyzeRegistraion();
                    popup.StartPosition = FormStartPosition.CenterParent;
                    popup.gReportNo = spdSampleList_Sheet1.Cells[spdSampleList_Sheet1.ActiveRowIndex, (int)ANALYZE_LIST.REPORT_NO].Text;
        ...
        private void spdSampleList_ButtonClicked(object sender, EditorNotifyEventArgs e)
        {
            ...
            try
            {
                ...
                string sClaimNo = spdSampleList_Sheet1.Cells[e.Row, (int)ANALYZE_LIST.REPORT_NO].Text;
                string sFileNo1 = "";
                string sFileNo2 = "";
                ...
                switch (e.Column)
                {
                    ...
                    case (int)ANALYZE_LIST.FILE_NO_02:
                        ...
                        if (e.Column == (int)ANALYZE_LIST.FILE_NO_01)
                        {
                            ...
                            if (spdSampleList_Sheet1.Cells[e.Row, e.Column].Tag != null)
                            {
                                //dialog.FileName = Environment.GetFolderPath(Environment.SpecialFolder.Startup).ToString() + "\\" + sClaimNo + "_고객불만통보서" + ".pdf";
                                dialog.FileName = "AnalyzeNo_" + sClaimNo + "_SampleInformation" + ".pdf";
                                dialog.Filter = "PDF (*.PDF) | *.PDF | 모든 파일 (*.*) | *.*";

                                if (dialog.ShowDialog() == DialogResult.OK)
                                {

                                    sFileNo1 = spdSampleList_Sheet1.Cells[e.Row, (int)ANALYZE_LIST.FILE_NO_01].Tag.ToString();
                        ...
                        else
                        {
                            ...
                            if (spdSampleList_Sheet1.Cells[e.Row, e.Column].Tag != null)
                            {
                                ...
                                dialog.FileName = "AnalyzeNo_" + sClaimNo + "_AnalyzeReport" + ".pdf";
                                dialog.Filter = "PDF (*.PDF) | *.PDF | 모든 파일 (*.*) | *.*";
                                ...
                                if (dialog.ShowDialog() == DialogResult.OK)
                                {
                                    ...
                                    sFileNo2 = spdSampleList_Sheet1.Cells[e.Row, (int)ANALYZE_LIST.FILE_NO_02].Tag.ToString();
