...
namespace CUS_QCM
{
    public partial class frmPopSpecList : CUS_COM.frmTranForm06
    {

        public frmPopSpecList()
        {
            InitializeComponent();
        ...
        private void UpdateSpecSetLimitInfo(TRSNode in_node, int iRow)
        {
            ...
            try
            {
                ...
                */
                ...
                list_item.AddString("CUST_TARGET_VALUE", MPCF.Trim(spdSpecList_Sheet1.Cells[iRow, (int)SPEC_INFO.CUST_TARGET_VALUE].Text));
                list_item.AddString("CUST_UPPER_SPEC_LIMIT", MPCF.Trim(spdSpecList_Sheet1.Cells[iRow, (int)SPEC_INFO.CUST_UPPER_SPEC_LIMIT].Text));
                list_item.AddString("CUST_LOWER_SPEC_LIMIT", MPCF.Trim(spdSpecList_Sheet1.Cells[iRow, (int)SPEC_INFO.CUST_LOWER_SPEC_LIMIT].Text));
        ...
        private void ViewLotList()
        {
            try
            {
                ...
                dvcArgu[2].sCondition_Value = txtMatID.Text + "%";

                dvcArgu[3].sCondition_ID = "MAT_DESC";
                dvcArgu[3].sCondition_Value = "%" + txtMatDesc.Text + "%";


                if (TPDR.GetDataOne("", ref dt, "CQCM1004-001", dvcArgu, false, false, ref sSql) == false)
                {
                    if (dt != null)
                        dt.Dispose();
                ...
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    spdSpecList_Sheet1.Cells[i, (int)SPEC_INFO.CUST_LOWER_SPEC_LIMIT].Value = dt.Rows[i]["CUST_LOWER_SPEC_LIMIT"];
                    spdSpecList_Sheet1.Cells[i, (int)SPEC_INFO.CUST_TARGET_VALUE].Value = dt.Rows[i]["CUST_TARGET_VALUE"];
                    spdSpecList_Sheet1.Cells[i, (int)SPEC_INFO.CUST_UPPER_SPEC_LIMIT].Value = dt.Rows[i]["CUST_UPPER_SPEC_LIMIT"];
        ...
        private void frmPopSpecList_Load(object sender, EventArgs e)
        {
            ...
            if (sArea_id != "")
            {
                ...
                txtMatID.Text = sMat_id;
                txtMatDesc.Text = sMat_desc;

                cdvDept.ReadOnly = true;
                cdvOper.ReadOnly = true;
                txtMatID.ReadOnly = true;
                txtMatDesc.ReadOnly = true;

                ViewLotList();

                //일단 베트남만 2중 스펙쓰고있어서..베트남만 보이게
                if (MPGV.gsFactory == "IJDV1")
                {
                    chkDualSpec.Checked = true;
                }
                else
                {
                    chkDualSpec.Checked = false;
                }
                pCustVisable();
            }
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            try
            {
                EXTFUC.SaveExcel(spdSpecList, "SpecData");  //spread 객체, 기본 파일명
        ...
        private void chkDualSpec_CheckedChanged(object sender, EventArgs e)
        {
            pCustVisable();
        }
        private void pCustVisable()
        {
            if (chkDualSpec.Checked == true)
            {
                spdSpecList_Sheet1.Columns.Get((int)SPEC_INFO.CUST_LOWER_SPEC_LIMIT).Visible = true;
                spdSpecList_Sheet1.Columns.Get((int)SPEC_INFO.CUST_TARGET_VALUE).Visible = true;
                spdSpecList_Sheet1.Columns.Get((int)SPEC_INFO.CUST_UPPER_SPEC_LIMIT).Visible = true;

            }
            else
            {
                spdSpecList_Sheet1.Columns.Get((int)SPEC_INFO.CUST_LOWER_SPEC_LIMIT).Visible = false;
                spdSpecList_Sheet1.Columns.Get((int)SPEC_INFO.CUST_TARGET_VALUE).Visible = false;
                spdSpecList_Sheet1.Columns.Get((int)SPEC_INFO.CUST_UPPER_SPEC_LIMIT).Visible = false;
            }
        }
        #endregion
    }
