...
namespace CUS_RAS
{
    public partial class frmViewCTMMixedPowerVacuumFurnaceData : CUS_COM.frmViewForm03
    {
        public frmViewCTMMixedPowerVacuumFurnaceData()
        {
            InitializeComponent();
        ...
        public static bool ViewOper(Control control, string sArea_ID)
        {
            try
            {
                ...
                if (TPDR.GetDataOne("", ref dt, "CRAS3015-000", dvcArgu, false, false, ref sSql) == false)
                {
                    if (dt != null)
                        dt.Dispose();
        ...
        public static bool ViewResID(Control control, string sOperID)
        {
            try
            {
                ...
                if (TPDR.GetDataOne("", ref dt, "CRAS3015-003", dvcArgu, false, false, ref sSql) == false)
                {
                    if (dt != null)
                        dt.Dispose();
        ...
        private void ViewFurnace1Data()
        {
            ...
            try
            {
                ...
                if (rdoDate.Checked)
                    sViewID = "CRAS3015-001";
                else
                    sViewID = "CRAS3015-002";
        ...
        private void ViewFurnace1Data_2()
        {
            ...
            try
            {
                ...
                if (cdvResId.Text == "E1000033")
                {
                    sql = " WITH InitialData AS ( " +
                                          "     SELECT  " +
                                          "         ' ' AS RES_ID, " +
                                          "         'CTM_PCD 진공열처리로 #1' AS RES_DESC,  " +
                                          "         CONVERT(VARCHAR(10), cre_date, 120) AS WORK_DATE,  " +
                                          "         cre_date AS WORK_TIME,   " +
                                          "         TEMP_SV AS SV_TEMP,  " +
                                          "         TEMP_PV AS PV_TEMP,  " +
                                          "         VACCUM AS PV_VACUUM,  " +
                                          "         OP_MODE AS RES_STATUS     " +
                                          "     FROM dbo.MIXED_POWER_VACCUM_1    " +
                ...
                else if(cdvResId.Text == "E1000032")
                {

                      sql = " WITH InitialData AS ( " +
                                "     SELECT  " +
                                "         ' ' AS RES_ID, " +
                                "         'CTM_PCBN 진공열처리로 #1' AS RES_DESC,  " +
                                "        CONVERT(VARCHAR(10), cre_date, 120) AS WORK_DATE,  " +
                                "        cre_date AS WORK_TIME,   " +
                                "         TEMP AS SV_TEMP,  " +
                                "         TEMP AS PV_TEMP,  " +
                                "         (Case when VACCUM_2 > -1.0 then format(VACCUM_1,'####.##0') else format(vaccum_1 / power(10,0-VACCUM_2),'####.##0') end) AS PV_VACUUM,  " +
                                "         MODE_START AS RES_STATUS     " +
                                "     FROM dbo.PCBN    " +
                    "     WHERE YEAR(cre_date) >= YEAR('" + start_time + "') " +
                        " ), " +
                        " FirstTimestamp AS ( " +
                ...
                */
                ...
                if (select_rs.HasRows)
                {
                    while (select_rs.Read())
                    {
                        ...
                        spdFurnace.ActiveSheet.Cells[iRows, (int)RAS_INFO.WORK_DATE].Text  = select_rs[2].ToString(); //MPCF.MakeDateFormat(select_rs[2].ToString(), DATE_TIME_FORMAT.DATE);
                        spdFurnace.ActiveSheet.Cells[iRows, (int)RAS_INFO.WORK_TIME].Text  = select_rs[3].ToString(); //MPCF.MakeDateFormat(select_rs[3].ToString(), DATE_TIME_FORMAT.DATETIME);
                        spdFurnace.ActiveSheet.Cells[iRows, (int)RAS_INFO.SV_TEMP].Value   = select_rs[4].ToString();
                        spdFurnace.ActiveSheet.Cells[iRows, (int)RAS_INFO.PV_TEMP].Value   = select_rs[5].ToString();
                        spdFurnace.ActiveSheet.Cells[iRows, (int)RAS_INFO.PV_VACUUM].Value = select_rs[6].ToString();
                        ...
                        spdFurnace.ActiveSheet.Cells[iRows, (int)RAS_INFO.RES_STATUS].Value = select_rs[7].ToString();
        ...
        private void frmViewCTMMixedPowerVacuumFurnaceData_Load(object sender, EventArgs e)
        {
            try
            {
