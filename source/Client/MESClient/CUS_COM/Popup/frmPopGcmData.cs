...
namespace CUS_COM
{
    public partial class frmPopGcmData : frmViewForm01
    {
        public frmPopGcmData()
        {
            InitializeComponent();
        ...
        private bool MakeColumnHeader()
        {
            ...
            try
            {
                ...
                lblFilterKey1.Visible = false;

                txtFilterKey2.Visible = false;
                lblFilterKey2.Visible = false;
                ...
                if (TABLE.GetString("KEY_1_PRT") != "")
                {
                    ...
                    lblFilterKey1.Visible = true;


                    lblFilterKey1.Text = TABLE.GetString("KEY_1_PRT");
                ...
                if (TABLE.GetString("KEY_2_PRT") != "")
                {
                    ...
                    lblFilterKey2.Visible = true;

                    lblFilterKey2.Text = TABLE.GetString("KEY_2_PRT");
        ...
        private bool ViewDataList(string[] sArgu)
        {
            ...
            try
            {
                ...
                in_node.ProcStep = '1';
                in_node.AddString("TABLE_NAME", txtGcmTable.Text);
        ...
        private void frmPopGcmData_Load(object sender, EventArgs e)
        {
            try
            {

                //메뉴에서 조회된건지 팝업에서 조회된건지.
                if (MenuSearch() == false)
                {

                    if (sTable_name_desc != "")
                    {
                        lblFormName.Text = sTable_name_desc;
                    }

                    if (sTable_name != "")
                    {
                        txtGcmTable.Text = sTable_name;
                        ViewTable();
        ...
        private bool MenuSearch()
        {
            //20220915 해당팝업 메뉴로 추가할수 있도록 추가 
            try
            {
                ...
                in_node.AddString("SQL", "SELECT B.DATA_1 AS CODE,C.FUNC_DESC AS NAME FROM "
                + " (SELECT MAX(FUNC_NAME) KEEP(dense_rank first order by UPDATE_TIME DESC) AS FUNC_NAME FROM CSECUSEDAT WHERE USER_ID ='" + MPGV.gsUserID + "') A "
                + " inner JOIN MGCMTBLDAT B  ON B.FACTORY ='" + MPGV.gsFactory + "' AND B.TABLE_NAME ='C_GCM_FUN_MENU' AND A.FUNC_NAME = B.KEY_1  "
                + " INNER JOIN MSECFUNDEF C  ON A.FUNC_NAME = C.FUNC_NAME ");
                ...
                do
                {
                   ...
                   txtGcmTable.Text =  out_node.GetList("ROWS")[0].GetList("COLS")[0].GetString("DATA");
                   lblFormName.Text = out_node.GetList("ROWS")[0].GetList("COLS")[1].GetString("DATA");
                    ViewTable();
                    btnView.PerformClick();
        ...
        private void frmPopGcmData_Activated(object sender, EventArgs e)
        {
            try
            {
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            try
            {
                EXTFUC.SaveExcel(spdData, "GCMData");  //spread 객체, 기본 파일명
