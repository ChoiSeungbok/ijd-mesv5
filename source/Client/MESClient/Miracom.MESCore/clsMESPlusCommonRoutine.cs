...
namespace Miracom.MESCore
{
    ...
    public sealed class MPCR
    {
        ...
        public static void SetGRPItem(string sItemName, string sLabelName, string sCodeViewName, Control parentControl, string[] sGrpTableName)
        {
            ...
            try
            {
                ...
                for (i = 0; i < out_node.GetList(0).Count; i++)
                {
                    ...
                    // Variable Group Table ??吏€?먰븯湲??꾪빐??蹂€寃?
                    // MWIPFACCMF.TBL ??吏€?뺣맂Table Name??議댁옱?쒕떎硫?洹?Table??Item?쇰줈媛믪쓣?쒖떆
        ...
        // 이 함수는 ChangeControlEnabled() 함수로 대체됨. 이전 호환성을 위해 남겨짐.
        ...
        //tabpage안에 포함된 모든 필드를 Disable한다
        ...
        public static int Client_Upgrade(int iStep)
        {
            ...
            try
            {
               if (MPIF.gInit.IsAvailableSendMessage == false) return 0;
                ...
                // Server 가 죽어있는 경우 주기적으로 Version 을 체크하면 "Notfound Replier" 메시지를 화면에 계속 출력함.
                // 그래서 아래와 같이 변경함.
                ...
                // iStep = 1 : 踰꾩쟾check ???€由щ㈃?낃렇?덉씠??
                // iStep = 2 : 踰꾩쟾???곴??놁씠?낃렇?덉씠??
                ...
                //Add by J.S. 2013.06.19 MSG를 이용한 upgrade방법 추가
                if (out_node.GetString("UPGRADE_METHOD") == "MSG")
                {
                    ...
                    //upgrade용 sub folder를 생성하고 필요파일을 복사한다.
                    ...
                    // Add by DM KIM 2013.08.28 Tibrv로 접속할 경우에도 MES Upgrade가 가능하도록 수정
        ...
        public static bool SEC_Login(ref bool UpgradeFlag)
        {
            ...
            try
            {
                ...
                //Upgrade???닿쾬?쇰줈Client??醫낅쪟瑜?援щ텇?쒕떎.
                in_node.AddString("PROGRAM_ID", Application.ProductName);
        ...
        private static void DownLoadCaptionFile()
        {
            try
            {
                TRSNode in_node = new TRSNode("VIEW_DOC_IN");
                TRSNode out_node = new TRSNode("VIEW_DOC_OUT");
                byte[] bt_buffer;
                string s_file_name;
                ...
                if (MPCR.CallService("CUS_BAS", "CUS_BAS_Update_Caption", in_node, ref out_node, true) == false)
                {
                    return;
                }
                ...
                {
                    s_file_name = "MESMessage.xml";
                    s_file_name = Application.StartupPath + "\\" + s_file_name;
        ...
        public static bool SEC_Login_Ext(ref bool UpgradeFlag)
        {
            ...
            try
            {
                ...
                /* 2009.12.11. Aiden. TRSNode 의 Member Name 을 비교할 경우 대/소문자를 구분할 지 여부를 설정 */
        ...
        */
        ...
        //       - Optional ByVal Oper As String ="" - Operation
        //       - Optional ByVal EventID As String ="" - Event ID
        //       - Optional byref LotResFlag as String - Lot_Or_Res_Flag 媛?c_step="1" ??寃쎌슦?먮쭔 ?꾩슂)
        //       - Optional ByRef spdData As FarPoint.Win.Spread.FpSpread = Nothing - SpreadSheet Control
        //
        ...
        /// <summary>
        /// BAS_SQL_Query_Out_Tag를 DataTable로 변환한다.
        /// </summary>
        ...
        /// <summary>
        /// List Control을 BAS_SQL_Query_Out_Tag 내용으로 채운다.
        /// </summary>
        ...
        /// <param name="obj">List Control</param>
        /// <param name="Query_Out">BAS_SQL_Query_Out_Tag</param>
        /// <param name="listview_icon_index">ListView인 경우 표시될 아이콘 인덱스</param>
        /// <param name="listview_generate_column_header">ListView인 경우 칼럼헤더를 쿼리문의 칼럼으로 설정할지 여부</param>
        ...
        public static bool CallService(string s_module_name, string s_service_name, TRSNode in_node, ref TRSNode out_node, string channel, DeliveryMode mode, bool b_ignore_message)
        {
            try
            {
                ...
                {
                    // 2009.04.08. Aiden.
                    // b_ignore_message = true 이더라도 통신중 발생한 에러는 화면에 표시하도록 한다.
        ...
        public static void ParseSequenceInfo(string s_string, ref string s_id, ref int i_seq)
        {
            ...
            s_id = "";
            i_seq = 0;
            /* Updated By YJJung 151211 Bug Fix Flow나 Material Desc 에 (,)가 포함된 경우 Error 발생*/
    ...
    public class clsDerivedChar
    {
        ...
        private void CalculateValue(int i_col_seq)
        {
            ...
            // Aiden. 2011.04.21. Exponential 값을 표시해야 하는 경우도 있으므로 가공하지 않고 표시하도록 변경
            ...
            if (MessageCaster.CallService("BAS", "BAS_SQL_Query", in_node, ref out_node) == true)
            {
                if (out_node.StatusValue == MPGC.MP_FAIL_STATUS)
                {
                    // 값이 입력되지 않은 경우 공백이므로 계산식에서 에러가 발생할 수 있다.
                    // 따라서 에러에 대해 무시하고 진행하도록 아래 문장을 주석처리
                    //MPCR.CheckContinueProc(out_node);
                    return;
                }
