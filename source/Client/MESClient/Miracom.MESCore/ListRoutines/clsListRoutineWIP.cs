...
//
//   Detail Description
//       - ViewFunctionList() : c_step = 1 : 紐⑤뱺 Function List瑜?媛?몄샂
//                              c_step = 2 : ?뱀젙 SecurityGroup???ы븿??Function List瑜?媛?몄샂
...
//       - ViewResUserList() :  c_step = 1 : Resource ? ?곌???紐⑤뱺 User List瑜?媛?몄샂
//                              c_step = 2 : ?뱀젙 User ? ?곌???紐⑤뱺 Resource List瑜?媛?몄샂
//
//       - ViewPrvGrpUserList() :  c_step = 1 : Privilege Group 怨??곌???紐⑤뱺 User List瑜?媛?몄샂
//                                 c_step = 2 : ?뱀젙 User ? ?곌???紐⑤뱺 Privilege Group List瑜?媛?몄샂
//
//       - ViewToolList() : c_step = '1' : Factory?댁쓽 紐⑤뱺 Tool List瑜?媛?몄삩??
//                          c_step = '2' : Factory, Tool_Type???대떦?섎뒗 Tool List瑜?媛?몄삩??
//
//                          delete_flag = ' ' : delete_flag???곴??놁씠 由ъ뒪?몃? 媛?몄삩??
//                          delete_flag = 'N' : delete_flag=' ' ??寃껊쭔 媛?몄삩??
//                          delete_flag = 'Y' : delete_flag<>' ' ??寃껊쭔 媛?몄삩??
//
//       - ViewToolList_Detail() : c_step = '1' : Factory?댁쓽 紐⑤뱺 Tool List瑜?媛?몄삩??
//                                 c_step = '2' : Factory, Tool_Type???대떦?섎뒗 Tool List瑜?媛?몄삩??
//
//                                 delete_flag = ' ' : delete_flag???곴??놁씠 由ъ뒪?몃? 媛?몄삩??
//                                 delete_flag = 'N' : delete_flag=' ' ??寃껊쭔 媛?몄삩??
...
namespace Miracom.MESCore
{
    public sealed class WIPLIST
    {
        ...
        //Area, sub_area 조회 조건 추가. _20200728_lwg
        public static bool ViewOperationList(Control control, char c_step, string sArea, string sSub_Area, bool sCus_flag)
        {
            return ViewOperationList(control, c_step, "", 0,"","",null, "", false, sArea, sSub_Area, sCus_flag);
        }

        //string sArea, string sSub_Area, bool sCus_flag 파라미터 추가. - 조회조건 추가로 인해. _ 20200728_lwg
        public static bool ViewOperationList(Control control, char c_step, string sMaterial, int iMatVer, string sFlow, string sFilter, TreeNode parentNode, string sExtFactory, bool b_sec_chk_flag, string sArea = "", string sSub_Area = "", bool sCus_flag = false)
        {
            ...
            //sArea, sSub_Area 파라미터 추가. _ 20200728_lwg
            in_node.AddString("AREA_ID", sArea);
            in_node.AddString("SUB_AREA_ID", sSub_Area);

            do
            {
                out_node = new TRSNode("VIEW_OPERATION_LIST_OUT");

                //sCus_flag 커스터 마이징 여부로 서비스 호출을 다르게 한다. _ 20200728_lwg
                if (sCus_flag == false)
                {
        ...
        //       - Optional ByVal sTranCode as string =""    : Transaction Code 紐?
        //
        public static bool ViewLotHistory(Control control, char c_step, string sLotID, string sFromTime, string sToTime, char sIncludeDelHistory, TreeNode parentNode, bool bIgnoreError, string sTranCode)
        {
            ...
            do
            {
                ...
                for (i = 0; i < out_node.GetList(0).Count; i++)
                {
                    ...
                    {
                        // 2007. 04.05. Aiden.
                        // View Lot history 에서이력을가져오고다시이함수를호출하여다시가져온다.
                        // 따라서동일한내용에대해서2번가져오므로아래함수를통해서한번에그릴수있도록함
        ...
        /* 2013.06.12. Aiden. Lot Extension 을 표시하는 기능 추가 */
        ...
        //      - c_step    : process step
        //
        /* 2013.06.12. Aiden. BIN List 에 Filter 추가 */
