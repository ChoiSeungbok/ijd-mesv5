...
namespace Miracom.MESCore
{
    public sealed class BOMLIST
    {
        ...
        //'        - ByVal c_step As String                        : 확장 Process Step
        //'        - Optional ByVal iImage_idx As Integer = -1    : List View에 들어갈 아이콘 인덱스
        //'        - Optional ByVal sExt_Factory As String = "": 현재 Factory가 아닌경우의 Factory
        //'        - Optional ByVal sTreeItem As String = ""    : TreeView 에서 추가할 Node의 Text
        //'       - Optional ByVal Col As Integer = -1        : Spread 컨트롤일 경우 데이타를 뿌려줄 Column Index (-1이면 특정 Row 전체에 뿌려줌)
        //'       - Optional ByVal Row As Integer = -1        : Spread 컨트롤일 경우 데이타를 뿌려줄 Row Index (-1이면 특정 Column 전체에 뿌려줌)
