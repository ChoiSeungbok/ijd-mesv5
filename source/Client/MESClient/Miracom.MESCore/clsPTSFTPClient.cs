...
namespace Miracom.MESCore
{
    ...
    {
        private string ftpServerIP, ftpUserID, ftpPassword, ftpPath;
        ...
        public PTSFTPClient() : this(string.Empty, string.Empty, string.Empty, string.Empty) { }

        public PTSFTPClient(string ServerIP, string UserID, string Password, string Path)
        {
        ...
        /// </summary>
        /// <param name="filename">file full name to be uploaded</param>
        private bool Upload2(string filename, string sChangeFileName = "")
        {
            FileInfo fileInf = new FileInfo(filename);
            string uri = "";

            if (MPCF.Trim(ftpPath) == string.Empty)
            {
                if (sChangeFileName == "")
                    uri = "ftp://" + ftpServerIP + "/" + fileInf.Name;
                else
                    uri = "ftp://" + ftpServerIP + "/" + sChangeFileName;

            }
            else
            {
                if (sChangeFileName == "")
                    uri = "ftp://" + ftpServerIP + "/" + ftpPath + "/" + fileInf.Name;
                else
                    uri = "ftp://" + ftpServerIP + "/" + ftpPath + "/" + sChangeFileName;

                string[] splitPath = ftpPath.Split('/');
                string path = "";
        ...
        public bool UpLoadFile(string fn, string sChangeFileName = "")
        {
            return Upload2(fn, sChangeFileName);
        }
        ...
        public bool DownLoadFile(string filePath, string fn)
        {
            if (Download(filePath, fn) == true)
                return true;
            else
                return false;
        }
