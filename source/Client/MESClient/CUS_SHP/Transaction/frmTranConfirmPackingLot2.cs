...
namespace CUS_SHP
{
    public partial class frmTranConfirmPackingLot2 : CUS_COM.frmTranForm01
    {
        public frmTranConfirmPackingLot2()
        {
            InitializeComponent();
        ...
        private void ViewPackingOrderList()
        {
            try
            {
                ...
                //       this.spdOrderList_Sheet1.Columns.Get((int)PACK_ORDER.CHK).Locked = true;
        ...
        //포장 정보 확정 취소
        private bool CancelConfirmPackInfo(int iRow)
        {
            ...
            try
            {
                ...
                in_node.ProcStep = '2';
                in_node.AddString("PACK_ORDER_ID", spdOrderList.ActiveSheet.Cells[iRow, (int)PACK_ORDER.PACK_ORDER_ID].Text);
                in_node.AddInt("LINE_NO", MPCF.ToInt(spdOrderList.ActiveSheet.Cells[iRow, (int)PACK_ORDER.LINE_NO].Text));


                ViewConfirmLot(spdOrderList.ActiveSheet.Cells[iRow, (int)PACK_ORDER.PACK_ORDER_ID].Text, spdOrderList.ActiveSheet.Cells[iRow, (int)PACK_ORDER.LINE_NO].Text, ref dt);
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    //확정되지 않은 LOT 체크
                    if (!string.Equals(dt.Rows[i]["PACK_FLAG"].ToString(), "Y"))
                    {
                        continue;
                    }

                    //이미 출하된 LOT 체크
                    if (string.Equals(dt.Rows[i]["SHIP_FLAG"].ToString(), "Y"))
                    {
                        continue;
                    }
                    ...
                    lot_list.AddString("ORG_LOT_ID", dt.Rows[i]["ORG_LOT_ID"].ToString());
                    lot_list.AddString("PACK_LOT_ID", dt.Rows[i]["PACK_LOT_ID"].ToString());
                    lot_list.AddString("CMF_2", dt.Rows[i]["CMF_2"].ToString());
                ...
                //CMN610 INFO -재출하시 포장LOT을 유지하시겠습니까?(이미 라벨이 부착된 상태면 YES, 그외경우면 NO를 부탁드립니다.)
                if (MPCF.ShowMsgBox(MPCF.GetMessage(610), MessageBoxButtons.YesNo, 2) != System.Windows.Forms.DialogResult.Yes)
                {
        ...
        private void ViewConfirmLot(string packOrderId, string packLineNo, ref DataTable dt)
        {
            try
            {
        ...
        //포장 정보 확정
        private bool ConfirmPackingLot(int iRow)
        {
        ...
        private void frmTranConfirmPackingLot2_Load(object sender, EventArgs e)
        {
            try
            {
                this.dtpToWorkDate.Value = DateTime.Now;
        ...
        private void btnCancel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (spdOrderList.ActiveSheet.Cells[i, (int)PACK_ORDER.CHK].Value.ToString().ToUpper() == "TRUE")
                    {
                        if (CancelConfirmPackInfo(i) == true) 
                        {
                            spdOrderList.ActiveSheet.Cells[i, (int)PACK_ORDER.CONFRIM_YN].Text = "Y";
                        }
                        else
                        {
                            spdOrderList.ActiveSheet.Cells[i, (int)PACK_ORDER.CONFRIM_YN].Text = "N";
                        }
                    }
                ...
                MPCF.ShowMsgBox("처리되었습니다.(Success Complete)");
