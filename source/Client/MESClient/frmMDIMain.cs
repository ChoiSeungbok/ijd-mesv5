...
namespace MESClient
{
    public partial class frmMDIMain : frmMDIMainCore, intMdiFormFunction
    {
        ...
        {
            InitializeComponent();
            MPGV.geLanguageFormat = LANG_FORMAT.STANDARD;
            MPGV.gsDateDelimiter = "-";
        }

        private const int SW_SHOWNORMAL = 1;
        private const int SW_MAXIMIZE = 3;
        private const int SW_SHOW = 5;
        private bool b_process_to_generate_menu_new = false;
        private List<string> allMenuItems;
        private ListBox listBoxSuggestions; // 자동완성 리스트 박스
        ...
        private void frmMDIMain_Activated(object sender, System.EventArgs e)
        {
            ...
            if (b_load_flag == false)
            {
                ...
                else
                {
                    ...
                    //20221222 패스워드 암호화하여 레지스트리에 저장(ID별)
                    Pssword_Encrypt();
                ...
                //RPT서버 채널
                TPDRCaster.RPTBASChannel = "/" + MPGV.gsSiteID + "/RPTServer";
                TPDRCaster.WIPTTL = MPGV.giTimeOut * 1000;
                ...
                //20221208
                //이거 안먹힘 SEC_Login_Ext 서비스에서 PASSWORD_CHANGE_FLAG 값을 정상적으로 가지고 오지를 않음
                //원인을 몰라서 별도로 로그인 Validation 만듬
                //회계 감사지적사항으로 패스워드 변경주기 90일로 체크하여야 하여 따로 조회해서 체크하는 방식으로 변경(IdChecking)
                //변경일 90일지나면 변경팝업뜨도록 수정.
                ...
                */
                ...
                MPGC.ShowWindow(this.Handle, SW_MAXIMIZE);
            }
        ...
        protected override bool InitMainForm()
        {
            try
            {
                ...
                if (GetAvailableFunctionListNew(mnuMain, 1, tolMain) == false) return false;
                ...
                {
                    AddCodeMenuText(mnuMain);
                }

                //   txtNavigator.ImeMode = ImeMode.Disable;

                try
                {
                    //"MES_V5_200701_01";
                    lblCusVer.Text = MPGV.gsClientVersion.Substring(9);
                }
                catch
                {
                }

                //글로벌 변수 AREA 코드를 가져온다._20201109_lwg
                CSCF.fnGetGlobalArea();

                //일단 메뉴리스트 다가져옴.syw
                allMenuItems = GetAllMenuItems(mnuMain.Items);
                txtNavigator.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
                txtNavigator.AutoCompleteSource = AutoCompleteSource.CustomSource;


                listBoxSuggestions = new ListBox();
                listBoxSuggestions.Visible = false;
                listBoxSuggestions.Height = 100;
                listBoxSuggestions.Width = txtNavigator.Width;
                listBoxSuggestions.IntegralHeight = false; // 스크롤 가능
                listBoxSuggestions.DoubleClick += ListBoxSuggestions_DoubleClick;
                listBoxSuggestions.KeyDown += ListBoxSuggestions_KeyDown;
                listBoxSuggestions.MouseClick += ListBoxSuggestions_MouseClick; // 마우스 클릭으로 메뉴 이동
                listBoxSuggestions.MouseDown += ListBoxSuggestions_MouseDown;


                // ListBox를 Form에 추가하고 txtNavigator 아래에 위치 syw
                this.Controls.Add(listBoxSuggestions);
                listBoxSuggestions.BringToFront();


                //if(MPGV.gsSiteID == "IDV1")
                if(MPGV.gsFactory == "IJDV1" || MPGV.gsSiteID == "IDV1")
                {
                    picflagKor.Visible = false;  //한국깃발
                    picflagVat.Visible = true;   //베트남깃발
                }
                else
                {
                    picflagKor.Visible = true;  //한국깃발
                    picflagVat.Visible = false; //베트남깃발
        ...
        public void AddCodeMenuText(MenuStrip mnuMenu)
        {
            if (mnuMenu == null) return;

            foreach (ToolStripMenuItem menu in mnuMenu.Items)
            {
                AddCodeMenuTextSub(menu);
            }
            //20241205 화면작아지면 MenuStrip 두새줄로...syw
            mnuMenu.LayoutStyle = ToolStripLayoutStyle.Flow;
        }

        private void AddCodeMenuTextSub(ToolStripMenuItem menu)
        {
            if (menu.Tag is MenuInfoTag)
            {
                MenuInfoTag mtags = (MenuInfoTag)menu.Tag;
                string s_func_name = mtags.s_func_name;

                if (mtags.c_func_type == 'F')
                {
                    menu.Text = "[" + s_func_name + "] " + menu.Text;
                }
            }
            ...
            for (int i = 0; i < menu.DropDownItems.Count; i++)
            {
                ...
                {
                    AddCodeMenuTextSub((ToolStripMenuItem)menu.DropDownItems[i]);
                }
            }
        ...
        //20230721 즐겨찾기 한글화 요청,실제 즐겨찾기메뉴명이 이미지 말고 글로 보이게 수정
        protected virtual bool ViewFavoritesListNew(ToolStrip tolTool)
        {
            ArrayList arrayList = new ArrayList();
            TRSNode tRSNode = new TRSNode("VIEW_FAVORITES_LIST_IN");
            int num = 0;
            TRSNode out_node;
            MenuInfoTag menuInfoTag = default(MenuInfoTag);
            string sFvName = "";
            try
            {
                MPCR.SetInMsg(tRSNode);
                tRSNode.ProcStep = '1';
                tRSNode.AddString("PROGRAM_ID", MPGV.gsProgramID);
                tRSNode.AddInt("NEXT_FUNC_SEQ", 0);
                do
                {
                    out_node = new TRSNode("VIEW_FAVORITES_LIST_OUT");
                    if (!MPCR.CallService("SEC", "SEC_View_Favorites_List", tRSNode, ref out_node))
                    {
                    ...
                    tRSNode.SetInt("NEXT_FUNC_SEQ", out_node.GetInt("NEXT_FUNC_SEQ"));
                }
                while (tRSNode.GetInt("NEXT_FUNC_SEQ") > 0);
                tolTool.Items.Clear();
                foreach (object item in arrayList)
                {
                    ...
                    for (int i = 0; i < out_node.GetList(0).Count; i++)
                    {
                        ...
                        menuInfoTag.s_func_name = out_node.GetList(0)[i].GetString("FUNC_NAME");
                        menuInfoTag.s_assembly_file = out_node.GetList(0)[i].GetString("ASSEMBLY_FILE");
                        menuInfoTag.c_func_type = out_node.GetList(0)[i].GetChar("FUNC_TYPE_FLAG");
                        if (menuInfoTag.c_func_type == 'P')
                        {
                            menuInfoTag.s_assembly_name = out_node.GetList(0)[i].GetString("ASSEMBLY_NAME");
                            menuInfoTag.s_args = null;
                        }
                        else
                        {
                            num = out_node.GetList(0)[i].GetString("ASSEMBLY_NAME").IndexOf(' ');
                            if (num > 0)
                            {
                                menuInfoTag.s_assembly_name = out_node.GetList(0)[i].GetString("ASSEMBLY_NAME").Substring(0, num);
                                menuInfoTag.s_args = out_node.GetList(0)[i].GetString("ASSEMBLY_NAME").Substring(num + 1, out_node.GetList(0)[i].GetString("ASSEMBLY_NAME").Length - num - 1);
                            }
                            else
                            {
                                menuInfoTag.s_assembly_name = out_node.GetList(0)[i].GetString("ASSEMBLY_NAME");
                        ...
                        sFvName = MPCF.FindLanguage(out_node.GetList(0)[i].GetString("USER_FUNC_DESC"), 2);
                        //toolStripButton.Text = sFvName;
                        if (sFvName.Length >= 12)
                        {
                            toolStripButton.Text = sFvName.Substring(0, 10) + "..";
                        }
                        else
                        {
                            toolStripButton.Text = sFvName;
                        ...
                        toolStripButton.ToolTipText = sFvName;
                        toolStripButton.Tag = menuInfoTag;
                        if (out_node.GetList(0)[i].GetInt("ICON_INDEX") >= 0 && out_node.GetList(0)[i].GetInt("ICON_INDEX") < imlToolBar.Images.Count)
                        {
                            toolStripButton.Image = imlToolBar.Images[out_node.GetList(0)[i].GetInt("ICON_INDEX")];
                        }
                        else
                        {
                            toolStripButton.Image = imlToolBar.Images[116];
                        }

                        toolStripButton.DisplayStyle = System.Windows.Forms.ToolStripItemDisplayStyle.ImageAndText;
                        toolStripButton.TextImageRelation = System.Windows.Forms.TextImageRelation.ImageAboveText;
                        toolStripButton.Font = new System.Drawing.Font("맑은 고딕", 6F);

                        toolStripButton.Click += ToolStripMenuItem_Click;
                        tolTool.Items.Add(toolStripButton);
                    }
                }
            ...
            if (!b_process_to_generate_menu_new)
            {
                TRSNode tRSNode2 = new TRSNode("VIEW_FUNCTION_LIST_IN");
                TRSNode out_node2 = new TRSNode("VIEW_FUNCTION_LIST_OUT");
                try
                {
                    MPCR.SetInMsg(tRSNode2);
                    tRSNode2.ProcStep = '1';
                    tRSNode2.AddString("PROGRAM_ID", MPGV.gsProgramID);
                    tRSNode2.AddString("SEC_GRP_ID", MPGV.gsUserGroup);
                    tRSNode2.AddString("NEXT_FUNC_NAME", "");
                    do
                    {
                        if (!MPCR.CallService("SEC", "SEC_View_Function_List", tRSNode2, ref out_node2))
                        {
                            return false;
                        }
                        ...
                        for (int i = 0; i < out_node2.GetList(0).Count; i++)
                        {
                            ...
                            ToolStripButton toolStripButton = new ToolStripButton();
                            menuInfoTag.s_func_name = out_node2.GetList(0)[i].GetString("FUNC_NAME");
                            menuInfoTag.s_assembly_file = out_node2.GetList(0)[i].GetString("ASSEMBLY_FILE");
                            menuInfoTag.c_func_type = out_node2.GetList(0)[i].GetChar("FUNC_TYPE_FLAG");
                            if (menuInfoTag.c_func_type == 'P')
                            {
                                menuInfoTag.s_assembly_name = out_node2.GetList(0)[i].GetString("ASSEMBLY_NAME");
                                menuInfoTag.s_args = null;
                            }
                            else
                            {
                                num = out_node2.GetList(0)[i].GetString("ASSEMBLY_NAME").IndexOf(' ');
                                if (num > 0)
                                {
                                    menuInfoTag.s_assembly_name = out_node2.GetList(0)[i].GetString("ASSEMBLY_NAME").Substring(0, num);
                                    menuInfoTag.s_args = out_node2.GetList(0)[i].GetString("ASSEMBLY_NAME").Substring(num + 1, out_node.GetList(0)[i].GetString("ASSEMBLY_NAME").Length - num - 1);
                                }
                                else
                                {
                                    menuInfoTag.s_assembly_name = out_node2.GetList(0)[i].GetString("ASSEMBLY_NAME");
                            ...
                            //toolStripButton.Text = out_node.GetList(0)[i].GetString("FUNC_DESC"); // "";
                            //toolStripButton.Text = MPCF.FindLanguage(out_node.GetList(0)[i].GetString("FUNC_DESC"), 2); //out_node.GetList(0)[i].GetString("FUNC_DESC"); // "";
                            //toolStripButton.ToolTipText = out_node2.GetList(0)[i].GetString("FUNC_DESC");



                            sFvName = MPCF.FindLanguage(out_node.GetList(0)[i].GetString("FUNC_DESC"), 2);
                            //toolStripButton.Text = sFvName;
                            if (sFvName.Length >= 12)
                            {
                                toolStripButton.Text = sFvName.Substring(0, 10) + "..";
                            ...
                            {
                                toolStripButton.Image = imlToolBar.Images[out_node2.GetList(0)[i].GetInt("ICON_INDEX")];
                            }
                            else
                            {
                                toolStripButton.Image = imlToolBar.Images[116];
                        ...
                        tRSNode2.SetString("NEXT_FUNC_NAME", out_node2.GetString("NEXT_FUNC_NAME"));
                    }
                    while (tRSNode2.GetString("NEXT_FUNC_NAME") != "");
                }
                catch (Exception ex2)
                {
        ...
        //20230721 즐겨찾기 한글화 요청,실제 즐겨찾기메뉴명이 이미지 말고 글로 보이게 수정 syw
        protected virtual bool GetAvailableFunctionListNew(MenuStrip mnuMenu, int i_menu_insert_index, ToolStrip tolTool)
        {
            TRSNode tRSNode = new TRSNode("VIEW_FUNCTION_LIST_IN");
            TRSNode out_node = new TRSNode("VIEW_FUNCTION_LIST_OUT");
            b_process_to_generate_menu_new = true;
            ViewFavoritesListNew(tolTool);
            try
            {
                MPCR.SetInMsg(tRSNode);
                tRSNode.ProcStep = '1';
                tRSNode.AddString("PROGRAM_ID", MPGV.gsProgramID);
                tRSNode.AddString("SEC_GRP_ID", MPGV.gsUserGroup);
                tRSNode.AddString("NEXT_FUNC_NAME", "");
                ArrayList arrayList = new ArrayList();
                int i;
                do
                {
                    if (!MPCR.CallService("SEC", "SEC_View_Function_List", tRSNode, ref out_node))
                    {
                        return false;
                    }
                    ...
                    {
                        arrayList.Add(out_node.GetList(0)[i]);
                    }

                    tRSNode.SetString("NEXT_FUNC_NAME", out_node.GetString("NEXT_FUNC_NAME"));
                }
                while (tRSNode.GetString("NEXT_FUNC_NAME") != "");
                i = 0;
                ShowMainMenu(mnuMenu, i_menu_insert_index, tolTool, null, "1", arrayList, ref i);
            }
            catch (Exception ex)
            {
            ...
            b_process_to_generate_menu_new = false;
            return true;
        }
        ...
        private void txtNavigator_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                in_node.ProcStep = '1';
                in_node.AddString("SQL", "SELECT * FROM MSECFUNDEF  WHERE 1 = 1 AND FUNC_NAME = '" + txtNavigator.Text.Trim().ToUpper() + "' ");
                if (MPCR.CallService("BAS", "BAS_SQL_Query", in_node, ref out_node) == false)
                {
                    return;
                }

                DataTable dt = MPCR.ConvertToDataTable(out_node);

                if (dt != null && dt.Rows.Count > 0)
                {
                    string func_name = MPCF.Trim(dt.Rows[0]["FUNC_NAME"]);
                    string assembly_file = MPCF.Trim(dt.Rows[0]["ASSEMBLY_FILE"]);
                    string assembly_name = MPCF.Trim(dt.Rows[0]["ASSEMBLY_NAME"]);
                    string func_type_flag = MPCF.Trim(dt.Rows[0]["FUNC_TYPE_FLAG"]);
                    ...
                    if (MPCF.Trim(assembly_name) == "" || MPCF.Trim(assembly_file) == "")
                    {
                        return;
                    }

                    MenuInfoTag m_menu_tag = new MenuInfoTag();

                    m_menu_tag.s_func_name = func_name;
                    m_menu_tag.s_assembly_file = assembly_file;
                    int args_start_p = assembly_name.IndexOf(' ');
                    if (args_start_p > 0)
                    {
                        m_menu_tag.s_assembly_name = assembly_name.Substring(0, args_start_p);
                        m_menu_tag.s_args = assembly_name.Substring(args_start_p + 1, assembly_name.Length - args_start_p - 1);
                    }
                    else
                    {
                        m_menu_tag.s_assembly_name = assembly_name;
                        m_menu_tag.s_args = null;
                    }
                    m_menu_tag.c_func_type = func_type_flag[0];
        ...
        private List<string> GetAllMenuItems(ToolStripItemCollection items)
        {
            ...
            foreach (ToolStripItem item in items)
            {
                ...
                {
                    menuList.Add(menuItem.Text);

                    if (menuItem.DropDownItems.Count > 0)
                    {
                        menuList.AddRange(GetAllMenuItems(menuItem.DropDownItems));
                    }
                }
            }
            return menuList;
        ...
        private bool IdChecking2()
        {
            return true;
        }

        public static bool IdChecking()
        {
            ...
            try
            {
                MPCR.SetInMsg(in_node);
                in_node.ProcStep = '1';
                in_node.AddString("SQL", "SELECT (CASE WHEN  PASSWORD_CHANGE_PERIOD = 0 THEN TRUNC(SYSDATE) + 100 ELSE TRUNC(TO_DATE(PASSWORD_CHANGE_DATE,'YYYYMMDD') + PASSWORD_CHANGE_PERIOD) end)- TRUNC(SYSDATE) AS PERIOD_PASSWORD" +
                                        ", CHANGE_PASSWORD_FLAG, PASSWORD_FAIL_COUNT FROM MSECUSREXT WHERE USER_ID = '" + MPGV.gsUserID + "' ");
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    int period_password = MPCF.ToInt(dt.Rows[0]["PERIOD_PASSWORD"].ToString());
                    string change_pass_flag = MPCF.Trim(dt.Rows[0]["CHANGE_PASSWORD_FLAG"]);
                    int pass_fail_count = MPCF.ToInt(dt.Rows[0]["PASSWORD_FAIL_COUNT"]);


                    if (period_password <= 0)
                    {
                        MPCF.ShowMsgBox("비밀번호 변경일입니다. 비밀번호를 변경하세요.");


                        return true;

                    }
                    else if (period_password <= 5)
                    {
                        MPCF.ShowMsgBox("비밀번호 변경일이" + period_password + "일 남았습니다.");


                        return false;
                    }

                    if (change_pass_flag == "Y")
                    {
                        MPCF.ShowMsgBox("비밀번호를 변경해주세요");
                    ...
                    {
                        MPCF.ShowMsgBox("비밀번호를 5회이상틀리셨습니다,비밀번호를 변경해주세요");
                        return true;
                    }
        ...
        private void Pssword_Encrypt()
        {
            ...
            sEncrypt = passwordIn.Des_Encrypt(MPGV.gsPassword);
            MPCF.SaveRegSetting(Application.ProductName, "Settings", MPGV.gsFactory + "_" + MPGV.gsUserID, sEncrypt.ToString());
        ...
        private void txtNavigator_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                string input = txtNavigator.Text.ToLower();
                ...
                var matches = allMenuItems.Where(menu => menu.ToLower().Contains(input)).Take(10).ToList();

                if (matches.Count > 0)
                {
                    listBoxSuggestions.Items.Clear();
                    listBoxSuggestions.Items.AddRange(matches.ToArray());


                    Point txtNavigatorLocation = txtNavigator.Parent.PointToScreen(txtNavigator.Location);
                    Point formRelativeLocation = this.PointToClient(txtNavigatorLocation);



                    // 위치 조정 (txtNavigator의 위에 배치)
                    int listBoxHeight = listBoxSuggestions.Height;
                    int newTopPosition = formRelativeLocation.Y - listBoxHeight; //txtNavigator.Bottom + listBoxHeight;

                    // 만약 `txtNavigator`가 너무 위쪽에 있으면, 아래쪽에 표시
                    if (newTopPosition < 0)
                    {
                        newTopPosition = formRelativeLocation.Y + txtNavigator.Height;
                    }


                    //listBoxSuggestions.Location = new System.Drawing.Point(txtNavigator.Location.X +20, newTopPosition);

                    //listBoxSuggestions.Location = new System.Drawing.Point(txtNavigator.Left, 546);
                    listBoxSuggestions.Location = new Point(formRelativeLocation.X, newTopPosition);
                    listBoxSuggestions.Width = txtNavigator.Width;
        ...
        private void ListBoxSuggestions_MouseDown(object sender, MouseEventArgs e)
        {
            listBoxSuggestions.Focus(); //  클릭 전에 포커스를 유지하여 자동 닫힘 방지
        ...
        private void ListBoxSuggestions_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            {
                txtNavigator.Text = listBoxSuggestions.SelectedItem.ToString();
                listBoxSuggestions.Visible = false;
                SelectMenuItem(txtNavigator.Text); //  메뉴 실행
            }
        }
        ...
        private void ListBoxSuggestions_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter && listBoxSuggestions.SelectedItem != null)
            {
                ...
                listBoxSuggestions.Visible = false;
                SelectMenuItem(txtNavigator.Text); // ✅ 메뉴 실행
                e.SuppressKeyPress = true; // Enter 키 이벤트 방지
            }
        }
        ...
        private void SelectSuggestion()
        {
            if (listBoxSuggestions.SelectedItem != null)
            {
                txtNavigator.Text = listBoxSuggestions.SelectedItem.ToString();
                listBoxSuggestions.Visible = false;
                SelectMenuItem(txtNavigator.Text);
            }
        }

        private void txtNavigator_Leave(object sender, EventArgs e)
        {
            try
            {
                if (txtNavigator == null || listBoxSuggestions == null)
                {
                    return;
                }

                if (txtNavigator.Focused)
                {
                    listBoxSuggestions.Visible = true;
                }
                else
                {
                    if (listBoxSuggestions.Focused)
                    {
                        listBoxSuggestions.Visible = true;
                    }
                    else
            ...
            catch (NullReferenceException) // 예외 처리 추가
            {
                return;
            }
        }
        ...
        private void SelectMenuItem(string menuName)
        {
            foreach (ToolStripItem item in mnuMain.Items)
            {
                if (item is ToolStripMenuItem menuItem)
                {
                    if (menuItem.Text.Equals(menuName, StringComparison.OrdinalIgnoreCase))
                    {
                        menuItem.PerformClick(); //  해당 메뉴 클릭 실행
                        return;
                    }

                    if (menuItem.DropDownItems.Count > 0)
                    {
                        SelectMenuItemRecursive(menuItem.DropDownItems, menuName);
                    }
                }
            }
        }
        private void SelectMenuItemRecursive(ToolStripItemCollection items, string menuName)
        {
            foreach (ToolStripItem item in items)
            {
                if (item is ToolStripMenuItem menuItem)
        ...
        private void txtNavigator_ImeModeChanged(object sender, EventArgs e)
        {

        }

        private void txtNavigator_EnabledChanged(object sender, EventArgs e)
        {

        }

        private void txtNavigator_ChangeUICues(object sender, UICuesEventArgs e)
        {
