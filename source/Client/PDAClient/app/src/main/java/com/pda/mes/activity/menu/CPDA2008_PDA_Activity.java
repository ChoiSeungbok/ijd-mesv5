...
import com.pda.mes.h101.request.inv.InvHoldLotRequest;
...
public class CPDA2008_PDA_Activity extends ToolbarActivity {
    public static final String TAG = CPDA2008_PDA_Activity.class.getSimpleName();
    private List<DropDownModel> listHoldModels = new ArrayList<>(); //잠금코드
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cpda2008, TOOLBAR_TYPE_PUSH);
        ...
        e.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                ((TextView)findViewById(R.id.holdCode)).setText("");
    ...
    protected void onResume() {
        super.onResume();
        this.registerReceiver(new CPDA2008_PDA_Activity.ScanResultReceiver(), filter);
    }
    ...
    public void onClickHoldCode(View view) {
    ...
    public void onClickRun(View view) {
        ...
        String sHoldCode = ((TextView)findViewById(R.id.holdCode)).getText().toString();
        ...
        {
            showDialog(getString(R.string.alert_message_no_hold_code));
            return;
        }
    ...
    private void requestHoldCodeList() {
        String requestQuery
                = "SELECT KEY_1 HOLD_CODE, DATA_1 HOLD_DESC"
                + " FROM MGCMTBLDAT"
                + " WHERE 1 = 1"
                + " AND FACTORY = '" + AppSettingManager.getLoginFactory(this) + "'"
                + " AND TABLE_NAME = \'C_INV_HOLD_CODE\'";
                ...
                if(resultMap.containsKey("ROWS")) {
                    try {
                        for(Map<String, String> row : (List<Map<String, String>>)resultMap.get("ROWS")) {
                            ...
                            model.setId(row.get("HOLD_CODE"));
                            model.setName(row.get("HOLD_DESC"));

                            listHoldModels.add(model);
                        }
                    }
    ...
    private void showHoldCodeDialog() {
        ComboBoxDialog dialog = new ComboBoxDialog(this);
        dialog.setDialogTitle(getString(R.string.common_word_hold_code));
        dialog.setData(listHoldModels);
        dialog.setOnSelectItemListener(new ComboBoxDialog.OnSelectItemListener() {
            @Override
            public void onClickItem(DropDownModel model) {
                String sHoldCode = ((TextView)findViewById(R.id.holdCode)).getText().toString();

                if(sHoldCode.equalsIgnoreCase(model.getName()))
                    return;

                for (DropDownModel holdModel : listHoldModels)
                {
                    holdModel.setSelected(false);
                }

                model.setSelected(true);

                ((TextView)findViewById(R.id.holdCode)).setText(model.getName());
            }
        });
        dialog.showDialog();
    }
    ...
    private void requestRun() {
        ...
        String sHoldCode = "";
        InvHoldLotRequest request = new InvHoldLotRequest(this);

        request.addParam("PROCSTEP", "C", "1");

        for (DropDownModel holdModel : listHoldModels)
        {
            if(holdModel.isSelected()) {
                sHoldCode = holdModel.getId();
                break;
            }
        }

        request.addParam("HOLD_CODE", "S", sHoldCode);
