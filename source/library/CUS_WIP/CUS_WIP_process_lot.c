...
System      : MESplus
Module      : CUS_WIP
File Name   : CUS_WIP_process_lot.c
Description : lot 메인 프로세스

MES Version : 5.0

Function List
- step 1 : 작업시작 - LOT의 공정 시작을 한다.
- step 2 : 작업완료 - LOT의 공정 완료를 한다.
...
int CUS_WIP_Process_Lot_Validation(char* s_msg_code, TRSNode* in_node, TRSNode* out_node);
int CUS_WIP_PROCESS_LOT(char* s_msg_code, TRSNode* in_node, TRSNode* out_node);
...
*******************************************************************************/
int CUS_WIP_Process_Lot(TRSNode* in_node, TRSNode* out_node)
{
	...
	i_ret = CUS_WIP_PROCESS_LOT(s_msg_code, in_node, out_node);

	COM_out_msg_log_write(s_msg_code, "CUS_WIP_Process_Lot", out_node);
	if (i_ret == MP_TRUE)
...
/*******************************************************************************
CUS_WIP_PROCESS_LOT()
- Main sub function of "CUS_WIP_Process_Lot" function
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
...
int CUS_WIP_PROCESS_LOT(char* s_msg_code, TRSNode* in_node, TRSNode* out_node)
{
	...
	struct MRASRESMFO_TAG MRASRESMFO;                   //설비 MFO
	struct CWIPOPRWRK_TAG CWIPOPRWRK;                   //공정 작업자
	struct CWIPLOTLST_TAG CWIPLOTLST;                   //불량 임시등록    
	struct MGCMTBLDAT_TAG MGCMTBLDAT;                   //GCM DATA TABLE (공정 옵션)
	...
	// VALIDATION CHECK
	if (CUS_WIP_Process_Lot_Validation(s_msg_code, in_node, out_node) == MP_FALSE)
	{
		COM_set_result(out_node, MP_FAIL_C, s_msg_code, MP_MSG_CATE_ERROR, TRS.get_language(in_node));
		return MP_FALSE;
	}
	...
	if (COM_isnullspace(TRS.get_string(in_node, "RES_ID")) == MP_FALSE)
	{
		/*
			...
			- RES_CMF_6 : 완료 LOT 전송 여부
						  => 완료 LOT 전송 여부가 'Y'인 경우 해당 LOT 정보를 설비에 보낸다.
							 ex) press설비가 완료 신호를 안보내주는 설비인 경우 mes에서 end 신호를 MC에 보내준다.
		*/
	...
	*/
		...
		for (int i = 0; i < i_lot_count; i++)
		{
				...
				*/
					...
					if (memcmp(MWIPOPRDEF_FROM.OPER, MWIPORDSTS.ORD_CMF_3, sizeof(MWIPOPRDEF_FROM.OPER)) != MP_FALSE)
					{
						...
						*/
						...
						if (MWIPLOTSTSX.INV_FLAG == 'Y')
						{
							...
							TRS.add_string(store_in, "TO_FLOW", MWIPORDSTS.FLOW, sizeof(MWIPORDSTS.FLOW));
					...
					//&& memcmp(MWIPOPRDEF_FROM.OPER, MWIPORDSTS.ORD_CMF_3, sizeof(MWIPOPRDEF_FROM.OPER)) == MP_FALSE)
				{
					...
					if (MWIPOPRDEF.OPER_CMF_14[0] != ' ')
					{
						...
						//double d_adpat_qty = COM_atof(MWIPOPRDEF.OPER_CMF_14[0], sizeof(MWIPOPRDEF.OPER_CMF_14[0]));

					   // TRS.add_double(adapt_in, "QTY_1", COM_atof(MWIPOPRDEF.OPER_CMF_14[0], sizeof(MWIPOPRDEF.OPER_CMF_14[0])));
						TRS.add_double(adapt_in, "QTY_1", COM_atof(MWIPOPRDEF.OPER_CMF_14, sizeof(MWIPOPRDEF.OPER_CMF_14)));
			...
			if (MWIPOPRDEF.OPER_CMF_14[0] != ' ')
			{
				...
				for (int i = 1; i <= MWIPLOTSTSX.QTY_1; i++)
				{
					...
					//  sprintf(s_lot_id, "%s%s%s", MWIPLOTSTSX.LOT_ID, ".", s_lot_seq[0]);

					sprintf(s_spl_lot_id, "%.*s%.*s%.*s", COM_len_space(MWIPLOTSTSX.LOT_ID, sizeof(MWIPLOTSTSX.LOT_ID)), MWIPLOTSTSX.LOT_ID, COM_len_space(".", sizeof(".")), ".", COM_len_space(s_lot_seq, sizeof(s_lot_seq)), s_lot_seq);
					...
					TRS.add_string(split_in, "CHILD_LOT_ID", s_spl_lot_id, sizeof(s_lot_id));
					TRS.add_string(split_in, "CHILD_LOT_DESC", s_spl_lot_id, sizeof(s_lot_id));
					...
					memcpy(MWIPLOTSTSX_SPLIT.LOT_ID, s_spl_lot_id, sizeof(MWIPLOTSTSX.LOT_ID));
					...
					TRS.add_double(list_item, "QTY_1", MWIPLOTSTSX_SPLIT.QTY_1);
					TRS.add_string(list_item, "MAT_ID", MWIPLOTSTSX_SPLIT.MAT_ID, sizeof(MWIPLOTSTSX_SPLIT.MAT_ID));
					TRS.add_string(list_item, "MAT_DESC", MWIPMATDEF.MAT_DESC, sizeof(MWIPMATDEF.MAT_DESC));
					TRS.add_string(list_item, "START_TIME", MWIPLOTSTSX_SPLIT.START_TIME, sizeof(MWIPLOTSTSX_SPLIT.START_TIME));

					if (MWIPLOTSTSX_SPLIT.START_RES_ID[0] != ' ' && MRASRESDEF.RES_ID[0] != ' ')
					{
	...
	else if (TRS.get_procstep(in_node) == '2')
	{
		...
		for (int i = 0; i < i_lot_count; i++)
		{
			...
			//이것떄문에 문의가 많이와서... 주석 풀음(20230911)
			/*
			//START 설비와 END시 선택한 설비가 다르면 작업 진행이 되지 않도록 벨리데이션.
			if (COM_isnullspace(TRS.get_string(in_node, "RES_ID")) == MP_FALSE)
			{
				if (memcmp(MWIPLOTSTSX.START_RES_ID, MRASRESDEF.RES_ID, sizeof(MRASRESDEF.RES_ID)) != MP_FALSE)
				{
					//WIP-0624 : Lot의 시작 설비와 완료 설비가 다릅니다. 설비를 확인하세요.
					strcpy(s_msg_code, "WIP-0624");
					TRS.add_fieldmsg(out_node, "START_RES_ID", MP_STR, sizeof(MWIPLOTSTSX.START_RES_ID), MWIPLOTSTSX.START_RES_ID);
					TRS.add_fieldmsg(out_node, "END_RES_ID", MP_STR, sizeof(MRASRESDEF.RES_ID), MRASRESDEF.RES_ID);
					TRS.add_dberrmsg(out_node, DB_error_msg);
			...
			*/
			...
			if (COM_dbl_round(TRS.get_double(Lot_tbl[i], "GOOD_QTY_1") + TRS.get_double(Lot_tbl[i], "LOSS_QTY_1"), 5, 'U') != COM_dbl_round(MWIPLOTSTSX.QTY_1, 5, 'U'))
			{
				...
				if (memcmp(MGCMTBLDAT.DATA_1, MP_TRAN_CODE_CREATE, strlen(MP_TRAN_CODE_CREATE)) == MP_FALSE || MGCMTBLDAT.DATA_5[0] == 'Y')
				{
					...
					//if (COM_isnullspace(TRS.get_string(Lot_tbl[i], "in_node")) == MP_FALSE)

					/*
					if (COM_isnullspace(TRS.get_string(Lot_tbl[i], "COMMENT")) == MP_FALSE)
					{
						TRS.add_nstring(cv_in, "COMMENT", MWIPLOTSTSX.LAST_COMMENT, sizeof(MWIPLOTSTSX.LAST_COMMENT));
					}
					else
					{
						//COMMENT LOT별로 변경(2022/02/21)
					//	TRS.add_nstring(cv_in, "COMMENT", TRS.get_string(in_node, "COMMENT"));
						TRS.add_nstring(cv_in, "COMMENT", TRS.get_string(Lot_tbl[i], "COMMENT"));
			...
			TRS.add_nstring(inv_in, "COMMENT", TRS.get_string(Lot_tbl[i], "COMMENT"));
			...
			//수동 컵계량 공정인 경우 무조건 해당 공정에서 자재 사용을 올린다. 
			if (memcmp(MGCMTBLDAT.DATA_2, MP_TRAN_CODE_SPLIT, strlen(MP_TRAN_CODE_SPLIT)) == MP_FALSE
				&& MGCMTBLDAT.DATA_4[0] != ' ' && MGCMTBLDAT.DATA_6[0] != ' ')
			{
				TRS.add_char(inv_in, "MUST_ISSUE_YN", 'Y');
			}
			...
			if ((int)DBU_select_cwiplotlst_scalar(4, &CWIPLOTLST) > 0)
			{
				...
				if (memcmp(MRASRESDEF.RES_GRP_1, MP_RESS_GRP_PRESS, strlen(MP_RESS_GRP_PRESS)) == MP_FALSE)
				{
					...
					if (memcmp(MGCMTBLDAT.DATA_3, MP_ID_ROLE_WIP_PRESS_LOT_ID, strlen(MP_ID_ROLE_WIP_PRESS_LOT_ID)) == 0)
					{
						...
						TRS.copy(MWIPLOTHISX.OPER, sizeof(MWIPLOTHISX.OPER), in_node, "OPER");
						...
						memcpy(CSUMLOTDAT.LOT_ID, MWIPLOTSTSX.LOT_ID, sizeof(CSUMLOTDAT.LOT_ID));
						TRS.copy(CSUMLOTDAT.OPER, sizeof(CSUMLOTDAT.OPER), in_node, "OPER");
						CSUMLOTDAT.START_HIST_SEQ = MWIPLOTHISX.HIST_SEQ;
						DBU_select_csumlotdat(1, &CSUMLOTDAT);
						if (DB_error_code == DB_SUCCESS)
						{
							DB_init_condition(&DBC_Q_COND);
							memcpy(DBC_Q_COND.KEY_1, s_lot_id, sizeof(s_lot_id));
							DB_add_null_condition(&DBC_Q_COND, &DBC_Q_COND_N);

							DBU_update_csumlotdat(2, &CSUMLOTDAT);
							if (DB_error_code != DB_SUCCESS)
							{
								LOG_head("CSUMLOTDAT Update Error");
								LOG_add("LOT_ID", MP_STR, sizeof(CSUMLOTDAT.LOT_ID), CSUMLOTDAT.LOT_ID);
								LOG_add("START_HIST_SEQ", MP_INT, CSUMLOTDAT.START_HIST_SEQ);
								LOG_add("OPER", MP_STR, sizeof(CSUMLOTDAT.OPER), CSUMLOTDAT.OPER);
								COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_VIEW);
							}
						}
						else
						{
							LOG_head("CSUMLOTDAT Not Found");
							LOG_add("LOT_ID", MP_STR, sizeof(CSUMLOTDAT.LOT_ID), CSUMLOTDAT.LOT_ID);
							LOG_add("START_HIST_SEQ", MP_INT, CSUMLOTDAT.START_HIST_SEQ);
							LOG_add("OPER", MP_STR, sizeof(CSUMLOTDAT.OPER), CSUMLOTDAT.OPER);
							COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_VIEW);
				...
				if (memcmp(MGCMTBLDAT.DATA_2, MP_TRAN_CODE_SPLIT, strlen(MP_TRAN_CODE_SPLIT)) == MP_FALSE)
				{
					...
					for (int k = 0; k < i_Qty; k++)
					{
						...
						if (s_group_id[0] != ' ')
						{
							...
							memcpy(MWIPLOTHISX.LOT_CMF_2, MWIPLOTSTSX_SPLIT.LOT_CMF_2, sizeof(MWIPLOTHISX.LOT_CMF_2));
...
/*******************************************************************************
CUS_WIP_Process_Lot_Validation()
- Validation Check sub function of "CUS_WIP_PROCESS_LOT" function
Return Value
