...
System      : MESplus
Module      : CUS_WIP
File Name   : CUS_WIP_move_lot.c
Description : Tran move Lot function
...
int CUS_WIP_Move_Lot_Validation(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
int CUS_WIP_Move_Lot_Before_Transaction(char *s_msg_code, int msg_id, TRSNode *in_node, TRSNode *out_node);
int CUS_WIP_Move_Lot_After_Transaction(char *s_msg_code, int msg_id, TRSNode *in_node, TRSNode *out_node);
...
*******************************************************************************/
int CUS_WIP_Move_Lot(TRSNode *in_node, TRSNode *out_node)
{
    ...
    i_ret = CUS_WIP_MOVE_LOT(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "CUS_WIP_Move_Lot", out_node);
    if (i_ret == MP_TRUE)
...
/*******************************************************************************
CUS_WIP_MOVE_LOT()
- Main sub function of "CUS_WIP_Move_Lot" function
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
...
- TRSNode *out_node : Output Message structure
*******************************************************************************/
int CUS_WIP_MOVE_LOT(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
    ...
    LOG_head("CUS_WIP_Move_Lot");
    TRS.log_add_all_members(in_node);
    COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_VIEW);

    /* Validation Check */
    if (CUS_WIP_Move_Lot_Validation(s_msg_code, in_node, out_node) == MP_FALSE)
    {
        COM_set_result(out_node, MP_FAIL_C, s_msg_code, MP_MSG_CATE_ERROR, TRS.get_language(in_node));
        return MP_FALSE;
    }
    ...
    //추가 정보성 데이터
    TRS.add_double(move_in_node, "QTY_1", MWIPLOTSTS.QTY_1);
    TRS.add_double(move_in_node, "QTY_2", MWIPLOTSTS.QTY_2);
    TRS.add_double(move_in_node, "QTY_3", MWIPLOTSTS.QTY_3);
    ...
    for (j = 0; j < 5; j++)
    {
        for (i = 0; i < 20; i++)
        {
            ...
            TRS.add_nstring(move_in_node, s_member_name, TRS.get_string(in_node, s_member_name));
        }
    }

    if (CUS_WIP_Move_Lot_Before_Transaction(s_msg_code, 0, move_in_node, out_node) == MP_FALSE)
    {
        return MP_FALSE;
    }

    if (WIP_MOVE_LOT(s_msg_code, move_in_node, out_node) == MP_FALSE)
    {
        return MP_FALSE;
    }

    if (CUS_WIP_Move_Lot_After_Transaction(s_msg_code, 0, move_in_node, out_node) == MP_FALSE)
    {
        return MP_FALSE;
    }
...
/*******************************************************************************
CUS_WIP_Move_Lot_Validation()
- Validation Check sub function of "CUS_WIP_MOVE_LOT" function
Return Value
...
/*******************************************************************************
CUS_WIP_Move_Lot_Before_Transaction()
- Main sub function of "CUS_WIP_MOVE_LOT" function
Return Value
- int : 1 (MP_TRUE) or 0 (MP_FALSE)
Arguments
...
/*******************************************************************************
CUS_WIP_Move_Lot_After_Transaction()
- Main sub function of "CUS_WIP_MOVE_LOT" function
Return Value
- int : 1 (MP_TRUE) or 0 (MP_FALSE)
Arguments
