...
System      : MESplus
Module      : CUS_EIS
File Name   : CUS_EIS_collect_mdextruder_data.c
Description : MD 압출기 설비 데이터 수집
...
int CUS_EIS_COLLECT_MDEXTRUDER_DATA(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
int Validation_CUS_EIS_COLLECT_MDEXTRUDER_DATA(char *s_msg_code, TRSNode *in_node, struct MRASRESDEF_TAG *MRASRESDEF, TRSNode *out_node);

/*******************************************************************************
CUS_EIS_Collect_MdExtruder_Data()
- MD 압출기 설비 데이터 수집
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
...
*******************************************************************************/
int CUS_EIS_Collect_MdExtruder_Data(TRSNode *in_node, TRSNode *out_node)
{
    ...
    i_ret = CUS_EIS_COLLECT_MDEXTRUDER_DATA(s_msg_code, in_node, out_node);
    ...
    COM_out_msg_log_write(s_msg_code, "CUS_EIS_Collect_MdExtruder_Data", out_node);
...
/*******************************************************************************
CUS_EIS_COLLECT_MDEXTRUDER_DATA()
- Main sub function of "CUS_EIS_Collect_MdExtruder_Data" function
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
...
int CUS_EIS_COLLECT_MDEXTRUDER_DATA(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
	...
	struct CRASMDEXTD_TAG CRASMDEXTD;
	struct CRASMDEXTD_TAG CRASMDEXTD_START;
    ...
    LOG_head("CUS_EIS_COLLECT_MDEXTRUDER_DATA");
    TRS.log_add_all_members(in_node);
    COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_VIEW);

    if (Validation_CUS_EIS_COLLECT_MDEXTRUDER_DATA(s_msg_code, in_node, &MRASRESDEF, out_node) == MP_FALSE)
        return MP_FALSE;
    ...
    if (TRS.get_procstep(in_node) == '1')
    {
        ...
        {
            DBU_init_crasmdextd(&CRASMDEXTD);
            memcpy(CRASMDEXTD.RES_ID, MRASRESDEF.RES_ID, sizeof(CRASMDEXTD.RES_ID));
            DBU_select_crasmdextd(3, &CRASMDEXTD);
            if (DB_error_code == DB_SUCCESS)
            {
                if (CRASMDEXTD.RES_STATUS == '0')
                {
        ...
        DBU_init_crasmdextd(&CRASMDEXTD);
        memcpy(CRASMDEXTD.RES_ID, MRASRESDEF.RES_ID, sizeof(CRASMDEXTD.RES_ID));
		CRASMDEXTD.HIST_SEQ = MRASRESDEF.LAST_ACTIVE_HIST_SEQ;
        memcpy(CRASMDEXTD.WORK_TIME, work_date.s_sys_time, sizeof(work_date.s_sys_time));
        memcpy(CRASMDEXTD.FACTORY, MRASRESDEF.FACTORY, sizeof(CRASMDEXTD.FACTORY));
        memcpy(CRASMDEXTD.WORK_DATE, work_date.s_work_date, sizeof(work_date.s_work_date));
        memcpy(CRASMDEXTD.DAY_NIGHT, work_date.s_day_night, sizeof(work_date.s_day_night));

        if (COM_isnullspace(TRS.get_string(in_node, "SV_PRESSURE")) == MP_FALSE)
			CRASMDEXTD.SV_PRESSURE = atof(TRS.get_string(in_node, "SV_PRESSURE"));

        if (COM_isnullspace(TRS.get_string(in_node, "PV_PRESSURE")) == MP_FALSE)
			CRASMDEXTD.PV_PRESSURE = atof(TRS.get_string(in_node, "PV_PRESSURE"));




		if (COM_isnullspace(TRS.get_string(in_node, "SV_TEMP")) == MP_FALSE)
			CRASMDEXTD.SV_TEMP = atof(TRS.get_string(in_node, "SV_TEMP"));

		if (COM_isnullspace(TRS.get_string(in_node, "PV_TEMP")) == MP_FALSE)
			CRASMDEXTD.PV_TEMP = atof(TRS.get_string(in_node, "PV_TEMP"));





		CRASMDEXTD.RES_STATUS = TRS.get_char(in_node, "EQ_STS");
        
        //EQ_STS : 0 : 종료 , 1: 시작, 2: 작업중
        if (CRASMDEXTD.RES_STATUS == '1')
            memcpy(CRASMDEXTD.START_TIME, work_date.s_sys_time, sizeof(work_date.s_sys_time));
        else if (CRASMDEXTD.RES_STATUS == '2' || CRASMDEXTD.RES_STATUS == '0')
        { 
            DBU_init_crasmdextd(&CRASMDEXTD_START);
            memcpy(CRASMDEXTD_START.RES_ID, MRASRESDEF.RES_ID, sizeof(CRASMDEXTD_START.RES_ID));
            DBU_select_crasmdextd(2, &CRASMDEXTD_START);

            if (DB_error_code == DB_SUCCESS)
            {
                memcpy(CRASMDEXTD.START_TIME, CRASMDEXTD_START.START_TIME, sizeof(CRASMDEXTD.START_TIME));
            }

            if (CRASMDEXTD.RES_STATUS == '0')
                memcpy(CRASMDEXTD.END_TIME, work_date.s_sys_time, sizeof(work_date.s_sys_time));
        }

        memcpy(CRASMDEXTD.EVENT_ID, MRASRESEVN.EVENT_ID, sizeof(MRASRESEVN.EVENT_ID));

        TRS.copy(CRASMDEXTD.CREATE_USER_ID, sizeof(CRASMDEXTD.CREATE_USER_ID), in_node, IN_USERID);
        memcpy(CRASMDEXTD.CREATE_TIME, gs_sys_time, sizeof(CRASMDEXTD.CREATE_TIME));

        DBU_insert_crasmdextd(&CRASMDEXTD);
        if (DB_error_code != DB_SUCCESS)
        {
            TRS.add_fieldmsg(out_node, "CRASMDEXTD Insert", DT_NOVALUESTRING);
            TRS.add_fieldmsg(out_node, "RES_ID", DT_STRING, sizeof(CRASMDEXTD.RES_ID), CRASMDEXTD.RES_ID);
...
/*******************************************************************************
Validation_CUS_EIS_COLLECT_MDEXTRUDER_DATA()
- Main sub function of "CUS_EIS_COLLECT_MDEXTRUDER_DATA" function
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
