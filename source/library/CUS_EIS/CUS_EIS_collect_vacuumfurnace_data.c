...
System      : MESplus
Module      : CUS_EIS
File Name   : CUS_EIS_collect_vacuumfurnace_data.c
Description : 진공 열처리로 설비 데이터 수집
...
int CUS_EIS_COLLECT_VACUUMFURNACE_DATA(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
int Validation_CUS_EIS_COLLECT_VACUUMFURNACE_DATA(char *s_msg_code, TRSNode *in_node, struct MRASRESDEF_TAG *MRASRESDEF, TRSNode *out_node);

/*******************************************************************************
CUS_EIS_Collect_VacuumFurnace_Data()
- 진공 열처리로 설비 데이터 수집
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
...
*******************************************************************************/
int CUS_EIS_Collect_VacuumFurnace_Data(TRSNode *in_node, TRSNode *out_node)
{
    ...
    i_ret = CUS_EIS_COLLECT_VACUUMFURNACE_DATA(s_msg_code, in_node, out_node);
    ...
    COM_out_msg_log_write(s_msg_code, "CUS_EIS_Collect_VacuumFurnace_Data", out_node);
...
/*******************************************************************************
CUS_EIS_COLLECT_VACUUMFURNACE_DATA()
- Main sub function of "CUS_EIS_Collect_VacuumFurnace_Data" function
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
...
int CUS_EIS_COLLECT_VACUUMFURNACE_DATA(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
	...
	struct CRASVCMFNC_TAG CRASVCMFNC;
	struct CRASVCMFNC_TAG CRASVCMFNC_START;
    ...
    LOG_head("CUS_EIS_COLLECT_VACUUMFURNACE_DATA");
    TRS.log_add_all_members(in_node);
    COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_VIEW);

    if (Validation_CUS_EIS_COLLECT_VACUUMFURNACE_DATA(s_msg_code, in_node, &MRASRESDEF, out_node) == MP_FALSE)
        return MP_FALSE;
    ...
    if (TRS.get_procstep(in_node) == '1')
    {
        ...
        {
            DBU_init_crasvcmfnc(&CRASVCMFNC);
            memcpy(CRASVCMFNC.RES_ID, MRASRESDEF.RES_ID, sizeof(CRASVCMFNC.RES_ID));
            DBU_select_crasvcmfnc(3, &CRASVCMFNC);
            if (DB_error_code == DB_SUCCESS)
            {
                if (CRASVCMFNC.RES_STATUS == '0')
                {
        ...
        while (1)
        {
            ...
            {                
                TRS.add_nstring(value_list, "VALUE", TRS.get_string(in_node, "SV_TEMP"));
            }

            if (memcmp(MEDCCOLCHR.CHAR_ID, "PV_TEMP", strlen("PV_TEMP")) == MP_FALSE)
            {
                TRS.add_nstring(value_list, "VALUE", TRS.get_string(in_node, "PV_TEMP"));
            }

            if (memcmp(MEDCCOLCHR.CHAR_ID, "PV_VACUUM", strlen("PV_VACCUM")) == MP_FALSE)
            {
                TRS.add_nstring(value_list, "VALUE", TRS.get_string(in_node, "PV_VACUUM"));
            }
        }
        ...
        DBU_init_crasvcmfnc(&CRASVCMFNC);
        memcpy(CRASVCMFNC.RES_ID, MRASRESDEF.RES_ID, sizeof(CRASVCMFNC.RES_ID));
		CRASVCMFNC.HIST_SEQ = MRASRESDEF.LAST_ACTIVE_HIST_SEQ;
        memcpy(CRASVCMFNC.WORK_TIME, work_date.s_sys_time, sizeof(work_date.s_sys_time));
        memcpy(CRASVCMFNC.FACTORY, MRASRESDEF.FACTORY, sizeof(CRASVCMFNC.FACTORY));
        memcpy(CRASVCMFNC.WORK_DATE, work_date.s_work_date, sizeof(work_date.s_work_date));
        memcpy(CRASVCMFNC.DAY_NIGHT, work_date.s_day_night, sizeof(work_date.s_day_night));

        if (COM_isnullspace(TRS.get_string(in_node, "SV_TEMP")) == MP_FALSE)
			CRASVCMFNC.SV_TEMP = atof(TRS.get_string(in_node, "SV_TEMP"));

        if (COM_isnullspace(TRS.get_string(in_node, "PV_TEMP")) == MP_FALSE)
			CRASVCMFNC.PV_TEMP = atof(TRS.get_string(in_node, "PV_TEMP"));

        if (COM_isnullspace(TRS.get_string(in_node, "PV_VACUUM")) == MP_FALSE)
			CRASVCMFNC.PV_VACUUM = atof(TRS.get_string(in_node, "PV_VACUUM"));

		CRASVCMFNC.RES_STATUS = TRS.get_char(in_node, "EQ_STS");
        
        //EQ_STS : 0 : 종료 , 1: 시작, 2: 작업중
        if (CRASVCMFNC.RES_STATUS == '1')
            memcpy(CRASVCMFNC.START_TIME, work_date.s_sys_time, sizeof(work_date.s_sys_time));
        else if (CRASVCMFNC.RES_STATUS == '2' || CRASVCMFNC.RES_STATUS == '0')
        { 
            DBU_init_crasvcmfnc(&CRASVCMFNC_START);
            memcpy(CRASVCMFNC_START.RES_ID, MRASRESDEF.RES_ID, sizeof(CRASVCMFNC_START.RES_ID));
            DBU_select_crasvcmfnc(2, &CRASVCMFNC_START);

            if (DB_error_code == DB_SUCCESS)
            {
                memcpy(CRASVCMFNC.START_TIME, CRASVCMFNC_START.START_TIME, sizeof(CRASVCMFNC.START_TIME));
            }

            if (CRASVCMFNC.RES_STATUS == '0')
                memcpy(CRASVCMFNC.END_TIME, work_date.s_sys_time, sizeof(work_date.s_sys_time));
        }

        memcpy(CRASVCMFNC.EVENT_ID, MRASRESEVN.EVENT_ID, sizeof(MRASRESEVN.EVENT_ID));

        TRS.copy(CRASVCMFNC.CREATE_USER_ID, sizeof(CRASVCMFNC.CREATE_USER_ID), in_node, IN_USERID);
        memcpy(CRASVCMFNC.CREATE_TIME, gs_sys_time, sizeof(CRASVCMFNC.CREATE_TIME));

        DBU_insert_crasvcmfnc(&CRASVCMFNC);
        if (DB_error_code != DB_SUCCESS)
        {
            TRS.add_fieldmsg(out_node, "CRASVCMFNC Insert", DT_NOVALUESTRING);
            TRS.add_fieldmsg(out_node, "RES_ID", DT_STRING, sizeof(CRASVCMFNC.RES_ID), CRASVCMFNC.RES_ID);
...
/*******************************************************************************
Validation_CUS_EIS_COLLECT_VACUUMFURNACE_DATA()
- Main sub function of "CUS_EIS_COLLECT_VACUUMFURNACE_DATA" function
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
