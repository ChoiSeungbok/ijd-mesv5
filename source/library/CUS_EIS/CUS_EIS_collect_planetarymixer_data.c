...
System      : MESplus
Module      : CUS_EIS
File Name   : CUS_EIS_collect_planetarymixer_data.c
Description : Planetary Mixer 설비 데이터 수집
...
int CUS_EIS_COLLECT_PLANETARYMIXER_DATA(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
int Validation_CUS_EIS_COLLECT_PLANETARYMIXER_DATA(char *s_msg_code, TRSNode *in_node, struct MRASRESDEF_TAG *MRASRESDEF, TRSNode *out_node);

/*******************************************************************************
CUS_EIS_Collect_PlanetaryMixer_Data()
- Planetary Mixer 설비 데이터 수집
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
...
*******************************************************************************/
int CUS_EIS_Collect_PlanetaryMixer_Data(TRSNode *in_node, TRSNode *out_node)
{
    ...
    i_ret = CUS_EIS_COLLECT_PLANETARYMIXER_DATA(s_msg_code, in_node, out_node);
    ...
    COM_out_msg_log_write(s_msg_code, "CUS_EIS_Collect_PlanetaryMixer_Data", out_node);
...
/*******************************************************************************
CUS_EIS_COLLECT_PLANETARYMIXER_DATA()
- Main sub function of "CUS_EIS_Collect_PlanetaryMixer_Data" function
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
...
int CUS_EIS_COLLECT_PLANETARYMIXER_DATA(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
	...
	struct CRASPNTMIX_TAG CRASPNTMIX;
	struct CRASPNTMIX_TAG CRASPNTMIX_START;
    ...
    LOG_head("CUS_EIS_COLLECT_PLANETARYMIXER_DATA");
    TRS.log_add_all_members(in_node);
    COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_VIEW);

    if (Validation_CUS_EIS_COLLECT_PLANETARYMIXER_DATA(s_msg_code, in_node, &MRASRESDEF, out_node) == MP_FALSE)
        return MP_FALSE;
    ...
    if (TRS.get_procstep(in_node) == '1')
    {
        ...
        {
            DBU_init_craspntmix(&CRASPNTMIX);
            memcpy(CRASPNTMIX.RES_ID, MRASRESDEF.RES_ID, sizeof(CRASPNTMIX.RES_ID));
            DBU_select_craspntmix(3, &CRASPNTMIX);
            if (DB_error_code == DB_SUCCESS)
            {
                if (CRASPNTMIX.RES_STATUS == '0')
                {
        ...
        DBU_init_craspntmix(&CRASPNTMIX);
        memcpy(CRASPNTMIX.RES_ID, MRASRESDEF.RES_ID, sizeof(CRASPNTMIX.RES_ID));
		CRASPNTMIX.HIST_SEQ = MRASRESDEF.LAST_ACTIVE_HIST_SEQ;
        memcpy(CRASPNTMIX.WORK_TIME, work_date.s_sys_time, sizeof(work_date.s_sys_time));
        memcpy(CRASPNTMIX.FACTORY, MRASRESDEF.FACTORY, sizeof(CRASPNTMIX.FACTORY));
        memcpy(CRASPNTMIX.WORK_DATE, work_date.s_work_date, sizeof(work_date.s_work_date));
        memcpy(CRASPNTMIX.DAY_NIGHT, work_date.s_day_night, sizeof(work_date.s_day_night));

        if (COM_isnullspace(TRS.get_string(in_node, "SV_TEMP")) == MP_FALSE)
			CRASPNTMIX.SV_TEMP = atof(TRS.get_string(in_node, "SV_TEMP"));

        if (COM_isnullspace(TRS.get_string(in_node, "PV_TEMP")) == MP_FALSE)
			CRASPNTMIX.PV_TEMP = atof(TRS.get_string(in_node, "PV_TEMP"));

		CRASPNTMIX.RES_STATUS = TRS.get_char(in_node, "EQ_STS");
        
        //EQ_STS : 0 : 종료 , 1: 시작, 2: 작업중
        if (CRASPNTMIX.RES_STATUS == '1')
            memcpy(CRASPNTMIX.START_TIME, work_date.s_sys_time, sizeof(work_date.s_sys_time));
        else if (CRASPNTMIX.RES_STATUS == '2' || CRASPNTMIX.RES_STATUS == '0')
        { 
            DBU_init_craspntmix(&CRASPNTMIX_START);
            memcpy(CRASPNTMIX_START.RES_ID, MRASRESDEF.RES_ID, sizeof(CRASPNTMIX_START.RES_ID));
            DBU_select_craspntmix(2, &CRASPNTMIX_START);

            if (DB_error_code == DB_SUCCESS)
            {
                memcpy(CRASPNTMIX.START_TIME, CRASPNTMIX_START.START_TIME, sizeof(CRASPNTMIX.START_TIME));
            }

            if (CRASPNTMIX.RES_STATUS == '0')
                memcpy(CRASPNTMIX.END_TIME, work_date.s_sys_time, sizeof(work_date.s_sys_time));
        }

        memcpy(CRASPNTMIX.EVENT_ID, MRASRESEVN.EVENT_ID, sizeof(MRASRESEVN.EVENT_ID));

        TRS.copy(CRASPNTMIX.CREATE_USER_ID, sizeof(CRASPNTMIX.CREATE_USER_ID), in_node, IN_USERID);
        memcpy(CRASPNTMIX.CREATE_TIME, gs_sys_time, sizeof(CRASPNTMIX.CREATE_TIME));

        DBU_insert_craspntmix(&CRASPNTMIX);
        if (DB_error_code != DB_SUCCESS)
        {
            TRS.add_fieldmsg(out_node, "CRASPNTMIX Insert", DT_NOVALUESTRING);
            TRS.add_fieldmsg(out_node, "RES_ID", DT_STRING, sizeof(CRASPNTMIX.RES_ID), CRASPNTMIX.RES_ID);
...
/*******************************************************************************
Validation_CUS_EIS_COLLECT_PLANETARYMIXER_DATA()
- Main sub function of "CUS_EIS_COLLECT_PLANETARYMIXER_DATA" function
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
