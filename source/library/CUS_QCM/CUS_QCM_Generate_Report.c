...
System      : MESplus
Module      : CUS_QCM
File Name   : CUS_QCM_Generate_Report.c
...
int CUS_QCM_Generate_Report_Validation(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
...
*******************************************************************************/
int CUS_QCM_Generate_Report(TRSNode *in_node, TRSNode *out_node)
{
    ...
    i_ret = CUS_QCM_GENERATE_REPORT(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "CUS_QCM_Generate_Report", out_node);
    if (i_ret == MP_TRUE)
...
/*******************************************************************************
CUS_QCM_GENERATE_REPORT()
- Main sub function of "CUS_QCM_Generate_Report" function
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
...
- TRSNode *out_node : Output Message structure
*******************************************************************************/
int CUS_QCM_GENERATE_REPORT(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
	//1. CTM 성적서 
    ...
    LOG_head("CUS_QCM_Generate_Report");
    ...
    /* Validation Check */
    if (CUS_QCM_Generate_Report_Validation(s_msg_code, in_node, out_node) == MP_FALSE)
    {
        COM_set_result(out_node, MP_FAIL_C, s_msg_code, MP_MSG_CATE_ERROR, TRS.get_language(in_node));
        return MP_FALSE;
    }
	...
	if (TRS.get_procstep(in_node) == '1')
	{
		...
		if (TRS.mem_cmp(in_node, "AREA_ID", gs_area_ctm, sizeof(gs_area_ctm)) == 0)
		{
			gen_in_node = TRS.add_node(in_node, "gen_in_node");

			//INSP_ID 발번
			CopyDefaultMembers(gen_in_node, in_node);
			TRS.add_string(gen_in_node, "RULE_ID", MP_SHIP_REPORT_CTM, strlen(MP_SHIP_REPORT_CTM));
			...
			for (int i = 0; i < i_data_count; i++)
			{
				...
				memcpy(CQCMRPTCTM.REPORT_NO, s_report_no, sizeof(CQCMRPTCTM.REPORT_NO));
				...
				TRS.copy(CQCMRPTCTM.FIT_STANDARD, sizeof(CQCMRPTCTM.FIT_STANDARD), data_list[i], "FIT_STANDARD");
				TRS.copy(CQCMRPTCTM.FIT_SPEC, sizeof(CQCMRPTCTM.FIT_SPEC), data_list[i], "FIT_SPEC");
				TRS.copy(CQCMRPTCTM.FIT_MIN_VAL, sizeof(CQCMRPTCTM.FIT_MIN_VAL), data_list[i], "FIT_MIN_VAL");
				TRS.copy(CQCMRPTCTM.FIT_MAX_VAL, sizeof(CQCMRPTCTM.FIT_MAX_VAL), data_list[i], "FIT_MAX_VAL");

				TRS.copy(CQCMRPTCTM.COMMENTS, sizeof(CQCMRPTCTM.OD_STANDARD), data_list[i], "COMMENTS");
	...
	else if (TRS.get_procstep(in_node) == '2') //HM
	{
	...
	else if (TRS.get_procstep(in_node) == '3') //GRIT
	{
		// TRS.get_string(in_node,"AREA_ID")


		if (TRS.mem_cmp(in_node, "AREA_ID", gs_area_grt, strlen(gs_area_grt)) == 0)
		{
			gen_in_node = TRS.add_node(in_node, "gen_in_node");

			//INSP_ID 발번
			CopyDefaultMembers(gen_in_node, in_node);
			TRS.add_string(gen_in_node, "RULE_ID", MP_SHIP_REPORT_GRIT, strlen(MP_SHIP_REPORT_GRIT));
			TRS.add_string(gen_in_node, "DATETIME", gs_sys_time, 8);
			TRS.add_string(gen_in_node, "OVR_TIME", gs_sys_time, 8);
			TRS.add_string(gen_in_node, "SEQ_KEY_10", gs_sys_time, 8);
			TRS.add_char(gen_in_node, IN_PROCSTEP, '2');
			...
			for (int i = 0; i < i_data_count; i++)
			{
				...
				TRS.copy(CQCMRPTGRN.FACTORY, sizeof(CQCMRPTGRN.FACTORY), in_node, IN_FACTORY);
				memcpy(CQCMRPTGRN.REPORT_NO, s_report_no, sizeof(CQCMRPTGRN.REPORT_NO));
				...
				TRS.copy(CQCMRPTGRN.ATTRIBUTE01, sizeof(CQCMRPTGRN.ATTRIBUTE01), data_list[i], "PSD_SS_SPEC");
				...
				TRS.copy(CQCMRPTGRN.ATTRIBUTE02, sizeof(CQCMRPTGRN.ATTRIBUTE02), data_list[i], "PSD_SS_VAL");
			...
			for (int i = 0; i < i_data_count; i++)
			{
				...
				TRS.copy(CQCMRPTGNP.FACTORY, sizeof(CQCMRPTGNP.FACTORY), in_node, IN_FACTORY);
				memcpy(CQCMRPTGNP.REPORT_NO, s_report_no, sizeof(CQCMRPTGNP.REPORT_NO));
				...
				if (DB_error_code != DB_SUCCESS)
				{
					...
					TRS.add_fieldmsg(out_node, "REPORT_NO", MP_STR, sizeof(CQCMRPTGNP.REPORT_NO), CQCMRPTGNP.REPORT_NO);
					TRS.add_fieldmsg(out_node, "SORT_NO", MP_INT, TRS.get_int(pivot_list[i], "SORT_NO"));
					TRS.add_dberrmsg(out_node, DB_error_msg);
			...
			for (int i = 0; i < i_data_count; i++)
			{
				...
				TRS.copy(CQCMRPTGNY.FACTORY, sizeof(CQCMRPTGNY.FACTORY), in_node, IN_FACTORY);
				memcpy(CQCMRPTGNY.REPORT_NO, s_report_no, sizeof(CQCMRPTGNY.REPORT_NO));
	...
	else if (TRS.get_procstep(in_node) == '4') //GRIT MP
	{
		// TRS.get_string(in_node,"AREA_ID")
		...
		if (TRS.mem_cmp(in_node, "AREA_ID", gs_area_grt, strlen(gs_area_grt)) == 0)
		{
			...
			for (int i = 0; i < i_data_count; i++)
			{
				...
				memcpy(CQCMRPTGRM.REPORT_NO, s_report_no, sizeof(CQCMRPTGRM.REPORT_NO));
			...
			memcpy(CQCMRPTGMP.REPORT_NO, s_report_no, sizeof(s_report_no));
