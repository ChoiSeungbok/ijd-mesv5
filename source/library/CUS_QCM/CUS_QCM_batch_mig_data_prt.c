...
System      : MESplus
Module      : CUS_QCM
File Name   : CUS_QCM_Batch_Mig_Data_Prt.c
Description :
...
int CUS_QCM_Batch_Mig_Data_Prt_Validation(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);


/*******************************************************************************
CUS_QCM_Batch_Mig_Data_Prt()
- Tran
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
...
*******************************************************************************/
int CUS_QCM_Batch_Mig_Data_Prt(TRSNode *in_node, TRSNode *out_node)
{
	...
	i_ret = CUS_QCM_BATCH_MIG_DATA_PRT(s_msg_code, in_node, out_node);

	COM_out_msg_log_write(s_msg_code, "CUS_QCM_Batch_Mig_Data_Prt", out_node);
	if (i_ret == MP_TRUE)
...
/*******************************************************************************
CUS_QCM_BATCH_MIG_DATA_PRT()
- Main sub function of "CUS_QCM_Batch_Mig_Data_Prt" function
Return Value
- int : 0 (IOI_SUCCESS)
Arguments
...
- TRSNode *out_node : Output Message structure
*******************************************************************************/
int CUS_QCM_BATCH_MIG_DATA_PRT(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
    char s_factory[10];
    char s_area[10];
    ...
    memset(s_report_no, 0x00, sizeof(s_report_no));
    ...
    LOG_head("CUS_QCM_Batch_Mig_Data_Prt");
    TRS.log_add_all_members(in_node);
    COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_VIEW);

    if (CUS_QCM_Batch_Mig_Data_Prt_Validation(s_msg_code, in_node, out_node) == MP_FALSE)
    {
        COM_set_result(out_node, MP_FAIL_C, s_msg_code, MP_MSG_CATE_ERROR, TRS.get_language(in_node));
        return MP_FALSE;
    }
    ...
    memcpy(s_report_no, TRS.get_string(in_node, "REPORT_NO"), strlen(TRS.get_string(in_node, "REPORT_NO")));
    ...
    DBU_batch_job_exec_qcm_prt_migration(s_factory, s_area, s_qcm_insp_type, s_insp_from_date, s_insp_to_date, s_insp_reproc_flag, s_mat_code, s_report_no, s_err_code, s_msg_rst_code, s_msg_rep_word);

    if (DB_error_code != DB_SUCCESS )
    {
        strcpy(s_msg_code, "QCM-0004");
        TRS.add_fieldmsg(out_node, "PR_MIG_QCM_DATA_RPT EXEC", DT_NOVALUESTRING);
...
/*******************************************************************************
CUS_QCM_Batch_Mig_Data_Prt_Validation()
- Validation Check sub function of "CUS_QCM_BATCH_MIG_DATA_PRT" function
Return Value
