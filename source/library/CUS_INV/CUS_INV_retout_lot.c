/*******************************************************************************
    ...
    System      : MESplus
    Module      : CUS_INV
    File Name   : CUS_INV_retout_lot.c
    Description : RET Out Inv Lot
    ...
    Function List
        - CUS_INV_RetOut_Lot()
            + Ret Out Lot
        - CUS_INV_RETOUT_LOT()
            + Main sub function of "CUS_INV_RetOut_Lot" function
            + Ret Out Lot definition
        - CUS_INV_RetOut_Lot_Validation()
            + Validation Check sub function of "CUS_INV_RETOUT_LOT" function
...
int CUS_INV_RETOUT_LOT(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
int CUS_INV_RetOut_Lot_Validation(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
int CUS_INV_RetOut_Lot_Before_Transaction(char *s_msg_code, int msg_id, TRSNode *in_node, TRSNode *out_node);
int CUS_INV_RetOut_Lot_After_Transaction(char *s_msg_code, int msg_id, TRSNode *in_node, TRSNode *out_node);
int CUS_INV_RETOUT_LOT_MAIN(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
...
*******************************************************************************/
int CUS_INV_RetOut_Lot(TRSNode *in_node,
                 TRSNode *out_node)
{   
    ...
    i_ret = CUS_INV_RETOUT_LOT_MAIN(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "CUS_INV_RETOUT_LOT", out_node);
...
- TRSNode *out_node : Output Message structure
*******************************************************************************/
int CUS_INV_RETOUT_LOT_MAIN(char *s_msg_code,
	TRSNode *in_node,
	TRSNode *out_node)
{
	TRSNode *retout_in_node;
	TRSNode **inv_lot_list;
	TRSNode *inv_list;
	int i;

	LOG_head("CUS_INV_RETOUT_LOT");
	COM_log_add_field_msg(in_node);
	TRS.log_add_all_members(in_node);
	COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_TRANS);

	/*' Validation Check */
	if (CUS_INV_RetOut_Lot_Validation(s_msg_code, in_node, out_node) == MP_FALSE)
	{
		COM_set_result(out_node, MP_FAIL_C, s_msg_code, MP_MSG_CATE_ERROR, TRS.get_language(in_node));
		return MP_FALSE;
	}

	retout_in_node = TRS.add_node(in_node, "retout_in_node");
	CopyDefaultMembers(retout_in_node, in_node);

	inv_lot_list = TRS.get_list(in_node, "INV_LOT_LIST");

	for (i = 0; i < TRS.get_item_count(in_node, "INV_LOT_LIST"); i++)
	{
		inv_list = TRS.add_node(retout_in_node, "INV_LOT_LIST");
		TRS.add_nstring(inv_list, "INV_LOT_ID", TRS.get_string(inv_lot_list[i], "INV_LOT_ID"));
	}

	TRS.add_nstring(retout_in_node, "TRAN_COMMENT", TRS.get_string(in_node, "TRAN_COMMENT"));

	if (CUS_INV_RetOut_Lot_Before_Transaction(s_msg_code, 0, in_node, out_node) == MP_FALSE)
	{
		return MP_FALSE;
	}

	if (CUS_INV_RETOUT_LOT(s_msg_code, in_node, out_node) == MP_FALSE)
	{
		return MP_FALSE;
	}

	if (CUS_INV_RetOut_Lot_After_Transaction(s_msg_code, 0, in_node, out_node) == MP_FALSE)
	{
		return MP_FALSE;
	}
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure    
*******************************************************************************/
int CUS_INV_RETOUT_LOT(char *s_msg_code,
                      TRSNode *in_node,
                      TRSNode *out_node)
{
	...
	for(i = 0; i < TRS.get_item_count(in_node, "INV_LOT_LIST") ; i++)
	{
		...
		// PROCSTEP = 1 -> 수입 검사를 통한 RET OUT
		// PROCSTEP = 2 -> 기타 출고 ETC OUT
		...
		if (TRS.get_procstep(in_node) == '1')
		{
			...
			memcpy(MINVLOTSTS.LAST_TRAN_CODE, MP_CINV_TRAN_CODE_RET_OUT, sizeof(MINVLOTSTS.LAST_TRAN_CODE));
		}
		else if (TRS.get_procstep(in_node) == '2')
		{
			...
			// 해당 자재 LOT 삭제
			MINVLOTSTS.DELETE_FLAG = 'Y';
			memcpy(MINVLOTSTS.DELETE_CODE, MP_CINV_TRAN_CODE_ETC_OUT, sizeof(MINVLOTSTS.DELETE_CODE));
			memcpy(MINVLOTSTS.DELETE_TIME, s_sys_time, sizeof(s_sys_time));
			TRS.copy(MINVLOTSTS.DELETE_USER_ID, sizeof(MINVLOTSTS.DELETE_USER_ID), in_node, IN_USERID);
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure
*******************************************************************************/
int CUS_INV_RetOut_Lot_Validation(char *s_msg_code,
                            TRSNode *in_node,
                            TRSNode *out_node)
{
    /*
...
/*******************************************************************************
CUS_INV_RetOut_Lot_Before_Transaction()
- Main sub function of "CUS_INV_RETOUT_LOT" function
Return Value
- int : 1 (MP_TRUE) or 0 (MP_FALSE)
Arguments
...
/*******************************************************************************
CUS_INV_RetOut_Lot_After_Transaction()
- Main sub function of "CUS_INV_RETOUT_LOT" function
Return Value
- int : 1 (MP_TRUE) or 0 (MP_FALSE)
Arguments
