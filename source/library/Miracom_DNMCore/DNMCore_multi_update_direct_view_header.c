/******************************************************************************'
	...
	System      : MESplus
	Module      : DNMCore
	File Name   : DNMCore_multi_update_direct_view_header.c
	Description : Direct_View_Header Setup function module

	MES Version : 5.2.0

	Function List
		- DNM_Multi_Update_Direct_View_Header()
			+ Create/Update/Delete Direct_View_Header definition
		- DNM_MULTI_UPDATE_DIRECT_VIEW_HEADER()
			+ Main sub function of DNM_Multi_Update_Direct_View_Header function
			+ Create/Update/Delete Direct_View_Header definition
		- DNM_Multi_Update_Direct_View_Header_Validation()
			+ Main sub function of DNM_MULTI_UPDATE_DIRECT_VIEW_HEADER function
...
int DNM_Multi_Update_Direct_View_Header_Validation(char *s_msg_code,
									TRSNode *in_node,
									TRSNode *out_node);
...
*******************************************************************************/
int DNM_Multi_Update_Direct_View_Header(TRSNode *in_node,
						TRSNode *out_node)
{
	...
	i_ret = DNM_MULTI_UPDATE_DIRECT_VIEW_HEADER(s_msg_code, in_node, out_node);

	COM_out_msg_log_write(s_msg_code,"DNM_MULTI_UPDATE_DIRECT_VIEW_HEADER", out_node);
...
/*******************************************************************************
	...
	Arguments
		...
		- TRSNode *out_node : Output Message structure
*******************************************************************************/
int DNM_MULTI_UPDATE_DIRECT_VIEW_HEADER(char *s_msg_code,
							  TRSNode *in_node, 
							  TRSNode *out_node)
{ 
...
//    TRSNode *update_in_node;
	...
	LOG_head("DNM_Multi_Update_Direct_View_Header");
	TRS.log_add_all_members(in_node);
	COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_SETUP);

	if(COM_proc_user_routine("DNM", "DNM_Multi_Update_Direct_View_Header",
							 MP_UPOINT_BEFORE,
							 in_node,
    ...
    for(i=0;i<TRS.get_item_count(in_node, "LIST");i++)
    {
        ...
        if(DNM_UPDATE_DIRECT_VIEW_HEADER(s_msg_code, List[i], out_node)==MP_FALSE)
        {
